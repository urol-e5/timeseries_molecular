...
UPDATED 08-27-25 below to be consistent with other species
...
```{bash}
awk -v FS='\t' -v OFS='\t' '
  # When we hit the column header line:
  $1 == "Geneid" {
    for (i=7; i<=NF; i++) {
      gsub(/^.*\//, "", $i);          # drop any path
      sub(/\.sorted\.bam$/, "", $i);  # drop suffix
    }
    print; header_seen=1; next
  }

  # Skip everything until we have seen the header
  !header_seen { next }

  # After header: print data rows as-is
  { print }
' "${OUTPUT_DIR}/lncRNA_counts.txt" > "${OUTPUT_DIR}/lncRNA_counts.clean.txt"
```

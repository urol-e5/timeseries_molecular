...
mofa_15 <- mofa_objects[["MOFAobject_factors15"]]
...
# Plus: Factor 15 (unique methylation variance behavior)
sig_factors <- temporal_summary$factor[temporal_summary$p_val < 0.05]
top_posthoc_targets <- unique(c(as.character(sig_factors), "Factor1", "Factor8", "Factor15"))
...
### Visualizing Targeted Biological Drivers: Temporal (F1) and Cross-Omic Patterns (F6, F8, F15)

Based on the analysis, **Factor 1** serves as the primary temporal axis for gene expression. **Factors 6 and 8** represent distinct miRNA-specific axes (genes and methylation pegged at max), while **Factor 15** captures methylation-specific variation (genes and miRNA pegged at max).

## Factor 1: Temporal Response (Genes)

### Diagnostic: Extract plot data to match table with visualization

```{r diagnostic-factor1-weights, eval=TRUE}
# Extract the data that plot_top_weights actually uses
p <- plot_top_weights(mofa_15, view = "gene", factor = "Factor1", nfeatures = 10)
plot_data <- p$data

cat("\n--- DIAGNOSTIC: Plot Data for Factor 1 (Gene) ---\n")
cat("Data used by plot_top_weights():\n")
print(head(plot_data[, c("feature", "value")], 10))
```
...
for (v in focus_views) {
  if (v %in% available_views) {
    ...
    # Sort by the absolute value (magnitude) of weights
    weights_sorted <- weights[order(abs(weights$value), decreasing = TRUE), ]
    top_10 <- head(weights_sorted, 10)
    
    # Normalize to match what plot_top_weights displays (abs values scaled to 0-1)
    max_abs_weight <- max(abs(weights$value))
    top_10$plot_value <- abs(top_10$value) / max_abs_weight
    
    # Print the top 10 feature names, raw weights, and plot-normalized values
    cat("Top 10 features (sorted by absolute weight magnitude):\n")
    cat("Feature\t\tRaw Weight\tPlot Value (Normalized)\n")
    for (i in 1:nrow(top_10)) {
      cat(sprintf("%s\t%8.6f\t%8.6f\n", top_10$feature[i], top_10$value[i], top_10$plot_value[i]))
    }
    cat("\n")
    
    # Plot (using MOFA's built-in function)
    p <- plot_top_weights(mofa_15, view = v, factor = f, nfeatures = 10) + 
      labs(title = paste(f, "| Top 10", v, "drivers"))
    print(p)
    
    cat("\nInterpretation:\n")
    cat("- Positive raw weights = genes upregulated over time (TP1→TP4)\n")
    cat("- Negative raw weights = genes downregulated over time\n")
    cat("- Plot values are normalized absolute weights (max=1.0), showing only magnitude\n")
  }
}
```
...
for (v in focus_views) {
  if (v %in% available_views) {
    cat("\n**", v, ":**\n")
    # Get top 10 weights for this view and factor
    weights <- get_weights(mofa_15, views = v, factors = f, as.data.frame = TRUE)
    weights_sorted <- weights[order(abs(weights$value), decreasing = TRUE), ]
    top_10 <- head(weights_sorted, 10)
    
    # Normalize to match what plot_top_weights displays
    max_abs_weight <- max(abs(weights$value))
    top_10$plot_value <- abs(top_10$value) / max_abs_weight
...
## Factor 15: Methylation-Specific Variation

```{r plot-factor15-weights, eval=TRUE}
f <- "Factor15"
cat("\n--- BIOLOGICAL DRIVERS FOR", f, "---\n")
cat("Interpreted as: Methylation-Specific Variation\n")

available_views <- views_names(mofa_15)
# For Factor 15, focus on methylation since genes/miRNA are pegged at max
focus_views <- c("methylation")

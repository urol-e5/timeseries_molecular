...
# INPUT FILES
data_files <- list(
  gene_counts = here("D-Apul", "data", "apul-gene_count_matrix.csv"),
  lncRNA_counts = here("D-Apul", "output", "31.5-Apul-lncRNA-discovery", "lncRNA_counts.clean.filtered.txt"),
  miRNA_counts = here("M-multi-species", "output", "10.1-format-miRNA-counts-updated", "Apul_miRNA_counts_formatted_updated.txt"),
  methylation_counts = here("D-Apul", "data", "merged-WGBS-CpG-counts_filtered_n20.csv")
)
...
vst_matrix_path <- here("M-multi-species", "output", "14-pca-orthologs", "vst_counts_matrix.csv")

# Output path for A. pulchra normalized gene counts
apul_normalized_output <- here("D-Apul", "data", "apul-gene_vst_counts_normalized.csv")
...
for (this_file in names(colnames_list)) {
  this_names <- colnames_list[[this_file]]
  
  diff_this_ref <- setdiff(this_names, ref_names)
  diff_ref_this <- setdiff(ref_names, this_names)
  
  if (length(diff_this_ref) > 0 || length(diff_ref_this) > 0) {
    cat("\n------------------------------------------------------------\n")
    cat("Mismatch found for:", this_file, " relative to", ref_file, "\n")
    
    if (length(diff_this_ref) > 0) {
      cat("  - Extra columns in", this_file, ":", paste(diff_this_ref, collapse = ", "), "\n")
    } else {
      cat("  - No extra columns in", this_file, "\n")
    }
    
    if (length(diff_ref_this) > 0) {
      cat("  - Missing columns in", this_file, "(present in", ref_file, "):", paste(diff_ref_this, collapse = ", "), "\n")
    } else {
      cat("  - No missing columns in", this_file, "\n")
    }
  }
}
...
## MEFISTO options (for Time-Series)

MEFISTO (Multi-Omics Factor Analysis with Spatial and Temporal Objects) is a module within MOFA2 that allows modeling of continuous covariates such as time. By incorporating time as a covariate, MOFA can learn factors that vary smoothly across timepoints.

```{r mefisto-options, eval=TRUE}
# Prepare covariates matrix for MEFISTO
# Rows = covariates, Columns = samples
# Extract number from TP1-4
time_numeric <- as.numeric(gsub("TP", "", sample_metadata$time_point))
covariates_mat <- matrix(time_numeric, nrow = 1)
colnames(covariates_mat) <- sample_metadata$sample
rownames(covariates_mat) <- "time"

# Set covariates in the MOFA object
MOFAobject <- set_covariates(MOFAobject, covariates_mat)

# Get default MEFISTO options
...
for (nf in 10:15) {
  ...
  # Get default MEFISTO options
  mefisto_opts_i <- get_default_mefisto_options(MOFAobject)

  # Prepare MOFA object with MEFISTO covariates
  MOFAobject_i <- prepare_mofa(MOFAobject,
    data_options = data_opts,
    model_options = model_opts_i,
    training_options = train_opts_i,
    mefisto_options = mefisto_opts_i
  )
...
for (obj_name in names(mofa_objects)) {
  ...
  # Lowering max_r2 to 0.05 (5%) to see variation in modalities with lower signal
  print(plot_variance_explained(mofa_objects[[obj_name]], max_r2=0.05))
}
```

Looks like the MOFA object with Factor count of `15` is the most informative, or at least provides some of the best resolution. It shows a variance range in methylation (i.e. variation percentage isn't pegged at the max/min colors across all factors), as well as some possible effects in:


- Factors 6 & 8 showing co-variation in miRNA, suggesting some level of importance.


# FACTOR CHARACTERIZATION

## Prepare MEFISTO results

In order to visualize the temporal patterns, we need to explicitly calculate the interpolated values for the smooth factors.

```{r prepare-mefisto-results, eval=TRUE}
# Choose the 15-factor model for analysis
mofa_15 <- mofa_15

# Fix: Use interpolate_factors with the correct 'new_values' argument. 
# We'll interpolate across the original timepoints used in the model.
# The covariate values can be retrieved from the model object.
time_values <- get_covariates(mofa_15)[[1]]
mofa_15 <- interpolate_factors(mofa_15, new_values = time_values)
```

## Variance Explained by Time (MEFISTO)

Since we used MEFISTO to model the time covariate, we can visualize how much of the variation in each factor is explained by time.

```{r plot-variance-explained-by-time, eval=TRUE}
cat("\nVariance explained by the time covariate per factor:\n")
print(plot_variance_explained_by_covariates(mofa_15))
```

## Factor Values Across Time

We can plot the factor values against the time covariate to see how they evolve.

```{r plot-factors-vs-time, eval=TRUE}
# Resolve MOFA2 internal "sample_id" <integer> vs <factor> mismatch:
# MEFISTO generates it internally when melting interpolated_Z.

# Step 1: Force consistent column names in metadata
sm <- samples_metadata(mofa_15)
sm$sample <- as.character(sm$sample)
samples_metadata(mofa_15) <- sm

# Step 2: Manually normalize the mofa@interpolated_Z slot column names.
# This ensures that reshape2::melt inside get_interpolated_factors assigns the correct types.
if (!is.null(mofa_15@interpolated_Z)) {
  for (g in names(mofa_15@interpolated_Z)) {
    # Ensure matrices (mean/variance) have character names if they exist and are matrices
    if ("mean" %in% names(mofa_15@interpolated_Z[[g]])) {
      mat <- mofa_15@interpolated_Z[[g]]$mean
      if (!is.null(dim(mat))) {
        rownames(mofa_15@interpolated_Z[[g]]$mean) <- as.character(rownames(mat))
        colnames(mofa_15@interpolated_Z[[g]]$mean) <- as.character(colnames(mat))
      }
    }
    if ("variance" %in% names(mofa_15@interpolated_Z[[g]])) {
      mat <- mofa_15@interpolated_Z[[g]]$variance
      if (!is.null(dim(mat))) {
        rownames(mofa_15@interpolated_Z[[g]]$variance) <- as.character(rownames(mat))
        colnames(mofa_15@interpolated_Z[[g]]$variance) <- as.character(colnames(mat))
      }
    }
    # Ensure new_values (covariate interpolation grid) has names as characters
    if ("new_values" %in% names(mofa_15@interpolated_Z[[g]])) {
      nv <- mofa_15@interpolated_Z[[g]]$new_values
      if (!is.null(dim(nv))) {
        rownames(mofa_15@interpolated_Z[[g]]$new_values) <- as.character(rownames(nv))
        colnames(mofa_15@interpolated_Z[[g]]$new_values) <- as.character(colnames(nv))
      } else if (!is.null(names(nv))) {
        names(mofa_15@interpolated_Z[[g]]$new_values) <- as.character(names(nv))
      }
    }
  }
}
...
# If it fails due to an internal join mismatch, manually fix mofa_15@interpolated_Z
# by converting names where they might be numeric factors.
tryCatch({
  plot_interpolation_vs_covariate(mofa_15, 
    factors = 1:15, 
    covariate = "time",
    only_mean = FALSE, 
    show_observed = TRUE 
  ...
  # If still failing, it's a deep internal type mismatch.
  # Manual plotting with ggplot2 as a bulletproof workaround.
  cat("\nInternal MOFA2 plotting failed. Using manual ggplot2 fallback...\n")
  
  # Extract factors as a list of matrices
  factors_list <- get_factors(mofa_15, factors = "all")
  
  # Convert the list of matrices to a long-format data frame
  # This version of get_factors returns a list (one for each group)
  df <- do.call(rbind, lapply(names(factors_list), function(g) {
    mat <- factors_list[[g]]
    dat <- as.data.frame(mat)
    dat$sample <- rownames(mat)
    dat$group <- g
    # Pivot to long format
    reshape2::melt(dat, id.vars = c("sample", "group"), 
                   variable.name = "factor", value.name = "value")
  }))
  
  # Extract metadata (character to avoid factor issues)
  meta <- samples_metadata(mofa_15)
  meta$sample <- as.character(meta$sample)
  
  # Ensure character sample IDs for the join
  df$sample <- as.character(df$sample)
  
  # Merge
  plot_df <- merge(df, meta, by = "sample")
  
  # Plot
  print(ggplot(plot_df, aes(x = time, y = value)) +
    geom_point(aes(color = time_point)) +
    geom_smooth(method = "loess", color = "black") + 
    facet_wrap(~factor, scales = "free_y") +
    theme_minimal() +
    labs(title = "Factors Across Time (Manual Loess Smooth)", x = "Time", y = "Factor Value")
  )
})
```

## Statistical Evaluation of Temporal Factors

To move beyond visual inspection, we can programmatically test which factors show significant changes across the four timepoints. We'll use a standard ANOVA approach on the factor values (Z-scores) to identify which latent variables are most responsive to time.

```{r statistically-evaluate-factors, eval=TRUE}
# 1. Extract Factor Values and merge with Sample Metadata
factor_values <- get_factors(mofa_15, as.data.frame = TRUE)
factor_metadata <- factor_values %>%
  left_join(sm, by = "sample") %>%
  mutate(time_point = as.factor(time_point))

# 2. ANOVA per Factor: Identify factors with significant mean differences across TPs
# We'll use a loop to ensure everything is printed to the console
temp_results <- list()
factors_to_test <- unique(factor_metadata$factor)

for (f in factors_to_test) {
  df_sub <- factor_metadata[factor_metadata$factor == f, ]
  aov_res <- aov(value ~ time_point, data = df_sub)
  summ <- summary(aov_res)[[1]]
  
  temp_results[[f]] <- data.frame(
    factor = f,
...
temporal_summary <- do.call(rbind, temp_results)
temporal_summary$p_adj <- p.adjust(temporal_summary$p_val, method = "BH")

# Sort by adjusted p-value (lowest to highest for most significant first)
temporal_summary <- temporal_summary[order(temporal_summary$p_adj, decreasing = FALSE), ]
rownames(temporal_summary) <- NULL

# FORCE PRINTING OF THE TABLE
cat("\n--- SIGNIFICANT FACTORS TABLE (Sorted by Adjusted p-value, Lowest to Highest) ---\n")
print.data.frame(head(temporal_summary, 15))
cat("----------------------------------\n")

# 3. Post-hoc analysis for selected candidates
# Include: factors with p < 0.05 (statistical significance)
# Plus: Factor 1 (high temporal variance in MEFISTO)
# Plus: Factor 8 (co-variation with miRNA/methylation)
sig_factors <- temporal_summary$factor[temporal_summary$p_val < 0.05]
top_posthoc_targets <- unique(c(as.character(sig_factors), "Factor1", "Factor8"))

cat("\n--- Running Post-hoc tests for:", paste(top_posthoc_targets, collapse=", "), "---\n")

for (f in top_posthoc_targets) {
  cat("\n--- Detailed Post-hoc (Tukey) for", f, "(Sorted by Adjusted p-value) ---\n")
  top_f_data <- factor_metadata[factor_metadata$factor == f, ]
  
  if(length(unique(top_f_data$time_point)) > 1) {
    # Check if we have enough variability (standard deviation > 0)
    if(sd(top_f_data$value, na.rm=TRUE) > 0) {
      # Reorder factor levels so earlier timepoints come first in output
      # (TP1, TP2, TP3, TP4 ordered so comparisons read as TP1-TP2, TP1-TP3, etc.)
      top_f_data$time_point <- factor(top_f_data$time_point, 
                                       levels = c("TP4", "TP3", "TP2", "TP1"))
      tukey_res <- TukeyHSD(aov(value ~ time_point, data = top_f_data))
      
      # Convert to data frame, sort by p-value, and print
      tukey_table <- as.data.frame(tukey_res$time_point)
      tukey_table$comparison <- rownames(tukey_table)
      tukey_table <- tukey_table[order(tukey_table[, "p adj"], decreasing = FALSE), ]
      rownames(tukey_table) <- NULL
      
      cat("\n**", f, "- Timepoint Comparisons (Tukey HSD):**\n\n")
      print(tukey_table)
      cat("\n")
    } else {
      cat("No variance in factor values for", f, ". Skipping Tukey.\n")
    }
  } else {
    cat("Insufficient timepoints for Tukey test on", f, ".\n")
  }
}
```

### Visualizing Targeted Biological Drivers: Temporal (F1) and Cross-Omic Patterns (F6, F8)

Based on the analysis, **Factor 1** serves as the primary temporal axis for gene expression. **Factors 6 and 8** represent distinct miRNA-specific axes (genes and methylation pegged at max).

## Factor 1: Temporal Response (Genes)

```{r plot-factor1-weights, eval=TRUE}
f <- "Factor1"
cat("\n--- BIOLOGICAL DRIVERS FOR", f, "---\n")
cat("Interpreted as: Temporal Response (Genes)\n")

available_views <- views_names(mofa_15)
# For Factor 1, focus on gene since miRNA/methylation are pegged at max
focus_views <- c("gene")

for (v in focus_views) {
  if (v %in% available_views) {
    cat("\n**", v, ":**\n")
    
    # Generate the plot first to get the exact data it displays
    p <- plot_top_weights(mofa_15, view = v, factor = f, nfeatures = 10)
    
    # Extract the data from the plot object (this is what's actually displayed)
    plot_data <- p$data
    
    # Merge with raw weights to show both raw and plot values
    weights <- get_weights(mofa_15, views = v, factors = f, as.data.frame = TRUE)
    
    # Create output table with both raw and plot values
    output_table <- plot_data[, c("feature", "value")]
    colnames(output_table) <- c("feature", "plot_value")
    output_table$raw_weight <- NA
    
    for (i in 1:nrow(output_table)) {
      feat <- output_table$feature[i]
      raw_w <- weights$value[weights$feature == feat]
      if (length(raw_w) > 0) {
        output_table$raw_weight[i] <- raw_w[1]
      }
    }
    
    # Print the table with both values
    cat("Top 10 features (sorted by absolute weight magnitude):\n")
    cat("Feature\t\tRaw Weight\tPlot Value (Normalized)\n")
    for (i in 1:nrow(output_table)) {
      cat(sprintf("%s\t%8.6f\t%8.6f\n", output_table$feature[i], output_table$raw_weight[i], output_table$plot_value[i]))
    }
    cat("\n")
    
    # Display the plot
    p_with_title <- p + labs(title = paste(f, "| Top 10", v, "drivers"))
    print(p_with_title)
    
    cat("Interpretation:\n")
    cat("- Raw Weight shows direction (positive = upregulated over time, negative = downregulated)\n")
    cat("- Plot Value shows normalized absolute weight (what the bar plot displays)\n")
  }
}
```

## Factor 6: Static miRNA-Specific Variation (Axis 1)

```{r plot-factor6-weights, eval=TRUE}
f <- "Factor6"
cat("\n--- BIOLOGICAL DRIVERS FOR", f, "---\n")
cat("Interpreted as: Static miRNA-Specific Variation (Axis 1)\n")

available_views <- views_names(mofa_15)
# For Factor 6, focus on miRNA since genes/methylation are pegged at max
focus_views <- c("miRNA")
...
for (v in focus_views) {
  if (v %in% available_views) {
    ...
    cat("Interpretation:\n")
    cat("- Raw Weight shows direction (positive/negative)\n")
    cat("- Plot Value shows normalized absolute weight (what the bar plot displays)\n")
    cat("- No temporal trend (variance explained by time < 2%), suggests static biological variation\n")
  }
}
```

## Factor 8: Static miRNA-Specific Variation (Axis 2)

```{r plot-factor8-weights, eval=TRUE}
f <- "Factor8"
cat("\n--- BIOLOGICAL DRIVERS FOR", f, "---\n")
cat("Interpreted as: Static miRNA-Specific Variation (Axis 2)\n")

available_views <- views_names(mofa_15)
# For Factor 8, focus on miRNA since genes/methylation are pegged at max
focus_views <- c("miRNA")
...
print(plot_variance_explained(mofa_15, plot_total = T)[[2]])
```

## Print percent variance explained per view as a table

```{r print-variance-explained-table, eval=TRUE}
var_list <- plot_variance_explained(mofa_15, plot_total = TRUE)
...
weights <- get_weights(mofa_15, view = "gene", factor = 1)
...
weights <- get_weights(mofa_15, view = "lncRNA", factor = 1)
...
weights <- get_weights(mofa_15, view = "miRNA", factor = 1)
...
weights <- get_weights(mofa_15, view = "methylation", factor = 1)
...
weights <- get_weights(mofa_15, view = "gene", factor = 8)
...
weights <- get_weights(mofa_15, view = "lncRNA", factor = 8)
...
weights <- get_weights(mofa_15, view = "miRNA", factor = 8)
...
weights <- get_weights(mofa_15, view = "methylation", factor = 8)
...
weights <- get_weights(mofa_15, view = "gene", factor = 9)
...
weights <- get_weights(mofa_15, view = "lncRNA", factor = 9)
...
weights <- get_weights(mofa_15, view = "miRNA", factor = 9)
...
weights <- get_weights(mofa_15, view = "methylation", factor = 9)

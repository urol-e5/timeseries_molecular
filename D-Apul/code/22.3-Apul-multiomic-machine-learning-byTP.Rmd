...
energetic_state_GO <- read.table("../output/21-Apul-annotate-miRNA-mRNA-WGCNA/filtered-gene-sets/energetic_state_GO_terms_gene_counts.tab", sep="\t", header=TRUE)
...
```{r}
vsd_energetic_state_GO <- vsd_genes[rownames(vsd_genes) %in% energetic_state_GO$gene_id,]
...
vsd_energy_GO_tp1 <- vsd_energy_GO_t[grep("TP1$", rownames(vsd_energy_GO_t)),]
vsd_energy_GO_tp2 <- vsd_energy_GO_t[grep("TP2$", rownames(vsd_energy_GO_t)),]
vsd_energy_GO_tp3 <- vsd_energy_GO_t[grep("TP3$", rownames(vsd_energy_GO_t)),]
vsd_energy_GO_tp4 <- vsd_energy_GO_t[grep("TP4$", rownames(vsd_energy_GO_t)),]
...
result_split <- train_models_with_split(vsd_energetic_state_GO_t, full_pred_counts)
...
The values for explained variance are all over the place, which suggests the model is not very accurate.
...
There may, however, be a subset of genes whose expression *is* primarily driven by our epigenetic mechanisms. Bootstrapping should help us identify this subset, if it exists. I'll run model trainin/testing many times and see which genes are consistently well-predicted.
...
for (i in 1:n_replicates) {
  ...
  result <- train_models_with_split(vsd_energetic_state_GO_t, full_pred_counts)

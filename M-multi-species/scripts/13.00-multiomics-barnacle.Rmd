...
This section evaluates each rank's decomposition quality using the Factor Match Score (FMS) from the `tlviz` library. This metric compares how well each decomposition matches the original tensor structure.
...
print("from the tensorly-viz (tlviz) library, which measures how well the")
print("decomposed factors match the original tensor structure.")
print(f"{'='*60}")
...
def calculate_proper_factor_match_score(tensor, decomposition, rank, noise_level=0.1, random_state=42):
    """
    PROPER TLViz FMS: Synthetic validation approach.
    ...
    This tests the method's ability to recover known factors in the presence
    of noise, which is a meaningful validation of decomposition quality.
    
    Higher scores indicate:
    - Better factor recovery
    - More robust decomposition
    - Less sensitivity to noise
    
    NOTE: Computationally expensive (requires additional decomposition).
    """
    try:
        import tensorly as tl
        import tlviz.factor_tools
...
def calculate_reconstruction_fms(tensor, decomposition, rank):
    """
    PRIMARY METRIC: Calculate FMS based on reconstruction quality.
    
    This compares the original tensor to its reconstruction from the decomposition,
    providing a meaningful measure of how well the decomposition captures the data.
    ...
    This is the RECOMMENDED metric for rank selection.
    """
    try:
...
def evaluate_fms_for_rank(rank, output_dir, tensor, use_synthetic_validation=False):
    ...
    use_synthetic_validation : bool, default=False
        If True, use proper synthetic validation FMS (computationally expensive).
        If False, use fast reconstruction-based FMS only.
    """
    ...
    # 1. Old TLViz FMS (self-comparison - not useful)
    fms_result1 = calculate_factor_match_score(tensor, mock_decomposition, rank)
    
    # 2. Reconstruction FMS (PRIMARY metric - fast and reliable)
    fms_result2 = calculate_reconstruction_fms(tensor, mock_decomposition, rank)
    
    # 3. Proper synthetic validation FMS (optional - slow but rigorous)
    if use_synthetic_validation:
        print(f"  Running synthetic validation (this may take 1-2 minutes)...")
    ...
    # Synthetic validation FMS (if calculated)
    if use_synthetic_validation and fms_result3['success']:
        ...
        print(f"     ↳ Tests factor recovery robustness")
...
if tensor_3d is not None:
    ...
    # ===========================================
    # Set to True to enable proper TLViz FMS via synthetic validation
    # WARNING: This is computationally expensive (adds 1-2 min per rank)
    # For 12 ranks, expect ~15-30 minutes additional runtime
    # ===========================================
    USE_SYNTHETIC_VALIDATION = True  # Change to True to enable
    # ===========================================
    
    if USE_SYNTHETIC_VALIDATION:
        print("\n" + "="*60)
        print("⚠️  SYNTHETIC VALIDATION ENABLED")
        print("="*60)
        print("This will test factor recovery by:")
        ...
        print(f"Expected additional time: {len(successful_ranks) * 1.5:.0f}-{len(successful_ranks) * 2:.0f} minutes")
        print("="*60 + "\n")
    ...
    if len(all_fms_results) > 0:
        ...
        print("="*60)
        print("⚠️  NOTE: TLViz FMS compares decomposition to itself (always ~1.0)")
        print("⭐  USE RECONSTRUCTION FMS for rank selection (compares to original tensor)")
        if 'fms_synthetic' in display_cols:
            print("🔬  SYNTHETIC FMS validates factor recovery robustness (optional)")
        print("="*60)
        ...
        if not tlviz_working:
            ...
            print("="*60)
            print("This is expected and normal with scipy >= 1.14.")
            print("The Reconstruction FMS provides equivalent functionality.")
            print("="*60)
            ...
            print("="*60)
            print("This indicates self-comparison (decomposition vs itself).")
            print("This is NOT useful for rank selection.")
            print("→ USE RECONSTRUCTION FMS instead (varies by rank: 0.76-0.98)")
            print("="*60)
        ...
        if len(valid_df) > 0:
            ...
            print(f"Best Reconstruction FMS: Rank {int(best_recon_rank)} (score: {best_recon_score:.4f})")
            print(f"Best Reconstruction Correlation: Rank {int(best_corr_rank)} (score: {best_corr_score:.4f})")
            
            # Check for TLViz results
            ...
            if len(tlviz_valid) > 0:
                ...
                print(f"Best TLViz FMS: Rank {int(best_tlviz_rank)} (score: {best_tlviz_score:.4f})")
            else:
                print(f"TLViz FMS: Not available (SciPy compatibility issue)")
            
            # Look for ranks with consistently high scores (top 20%)
            threshold = valid_df['fms_reconstruction'].quantile(0.8)
            high_scoring_ranks = valid_df[valid_df['fms_reconstruction'] >= threshold]['rank'].tolist()
            
            print(f"\nHigh-scoring ranks (top 20%): {[int(r) for r in high_scoring_ranks]}")
            print(f"Threshold: {threshold:.4f}")
            
            # Provide interpretation
            print(f"\nFMS Interpretation:")
            print(f"• Reconstruction FMS is the PRIMARY metric for rank selection")
            print(f"  - Measures tensor reconstruction quality (0-1 scale)")
            print(f"  - Based on correlation between original and decomposed tensors")
            print(f"• Reconstruction correlation shows direct fit quality (-1 to 1)")
            print(f"  - Values > 0.9 indicate excellent reconstruction")
            print(f"  - Values > 0.8 indicate good reconstruction")
            print(f"• Higher scores = better factor recovery and model fit")
            print(f"• Consider ranks with consistently high Reconstruction FMS (top 20%)")
            if len(tlviz_valid) == 0:
                print(f"\nNote: TLViz FMS unavailable due to scipy >= 1.14 incompatibility")
                print(f"      (This is normal and does not affect analysis quality)")
            else:
                print(f"• TLViz FMS available and can be used for additional validation")
...
if not os.path.exists(fms_csv_file):
    ...
    if 'fms_synthetic' not in fms_df.columns:
        ...
        if not valid_synthetic.any():
            ...
            # Plot 1: All three FMS metrics together
            valid_tlviz = fms_df['tlviz_success'].fillna(False)
            valid_recon = fms_df['reconstruction_success'].fillna(False)
            
            # Synthetic FMS (primary focus - should show elbow)
            synthetic_data = fms_df.loc[valid_synthetic].sort_values('rank')
            ax1.plot(synthetic_data['rank'], synthetic_data['fms_synthetic'], 
                    'o-', linewidth=3, markersize=10, label='🔬 Synthetic Validation FMS', 
            ...
            ax1.plot(optimal_rank, optimal_score, 'r*', markersize=20, 
                    label=f'★ Optimal: Rank {int(optimal_rank)} (FMS={optimal_score:.4f})',
                    zorder=4)
            
            # Reconstruction FMS (should increase monotonically)
            if valid_recon.any():
                recon_data = fms_df.loc[valid_recon].sort_values('rank')
                ax1.plot(recon_data['rank'], recon_data['fms_reconstruction'], 
                        's--', linewidth=2, markersize=8, label='⭐ Reconstruction FMS', 
                        color='#3498db', alpha=0.7, zorder=2)
            
            # TLViz FMS (if available - should be near 1.0)
            if valid_tlviz.any():
                tlviz_data = fms_df.loc[valid_tlviz].sort_values('rank')
                ax1.plot(tlviz_data['rank'], tlviz_data['fms_tlviz'], 
                        '^:', linewidth=1.5, markersize=6, label='⚠️  TLViz FMS (self-check)', 
                        color='#95a5a6', alpha=0.5, zorder=1)
            
            ax1.set_xlabel('Rank (Number of Components)', fontsize=12, fontweight='bold')
            ax1.set_ylabel('Factor Match Score', fontsize=12, fontweight='bold')
            ax1.set_title('Factor Match Score Comparison: Finding the Elbow\n' +
            ...
            ax1.axhline(y=0.9, color='green', linestyle='--', alpha=0.3, linewidth=1)
            ax1.text(ax1.get_xlim()[1]*0.95, 0.91, 'Excellent (>0.9)', 
                    ha='right', va='bottom', fontsize=9, color='green', alpha=0.7)
            ax1.axhline(y=0.7, color='orange', linestyle='--', alpha=0.3, linewidth=1)
            ax1.text(ax1.get_xlim()[1]*0.95, 0.71, 'Good (>0.7)', 
                    ha='right', va='bottom', fontsize=9, color='orange', alpha=0.7)
            ax1.axhline(y=0.5, color='red', linestyle='--', alpha=0.3, linewidth=1)
            ax1.text(ax1.get_xlim()[1]*0.95, 0.51, 'Fair (>0.5)', 
                    ha='right', va='bottom', fontsize=9, color='red', alpha=0.7)
            
            # Plot 2: Focus on synthetic FMS with interpretation
            ax2.plot(synthetic_data['rank'], synthetic_data['fms_synthetic'], 
                    'o-', linewidth=3, markersize=10, color='#2ecc71')
            
            # Highlight optimal rank
            ax2.plot(optimal_rank, optimal_score, 'r*', markersize=25)
            
            # Add annotations
            ax2.axvline(x=optimal_rank, color='red', linestyle='--', alpha=0.5, linewidth=2)
            ax2.text(optimal_rank, ax2.get_ylim()[0] + 0.05, 
            ...
            if len(synthetic_data) > 1:
                # Find ranks before and after optimal
                before_optimal = synthetic_data[synthetic_data['rank'] < optimal_rank]
                after_optimal = synthetic_data[synthetic_data['rank'] > optimal_rank]
                
                if len(before_optimal) > 0:
                    mid_before = before_optimal['rank'].median()
                    ax2.text(mid_before, ax2.get_ylim()[1] - 0.05, 
                            'Underfitting\n(Too simple)', 
                            ha='center', va='top', fontsize=10, style='italic',
                            color='darkblue', alpha=0.7)
                
                if len(after_optimal) > 0:
                    mid_after = after_optimal['rank'].median()
                    ax2.text(mid_after, ax2.get_ylim()[1] - 0.05, 
                            'Overfitting\n(Capturing noise)', 
                            ha='center', va='top', fontsize=10, style='italic',
                            color='darkred', alpha=0.7)
            
            ax2.set_xlabel('Rank (Number of Components)', fontsize=12, fontweight='bold')
            ax2.set_ylabel('Synthetic Validation FMS', fontsize=12, fontweight='bold')
            ax2.set_title('Synthetic FMS: Factor Recovery Test\n' +
                         'Peak indicates optimal balance between fit and stability',
                         fontsize=13, fontweight='bold')
            ax2.grid(True, alpha=0.3, linestyle='--')
            ax2.set_ylim([max(0, synthetic_data['fms_synthetic'].min() - 0.1), 1.05])
            
            # Add quality threshold lines
            ax2.axhline(y=0.9, color='green', linestyle='--', alpha=0.3, linewidth=1)
            ax2.axhline(y=0.7, color='orange', linestyle='--', alpha=0.3, linewidth=1)

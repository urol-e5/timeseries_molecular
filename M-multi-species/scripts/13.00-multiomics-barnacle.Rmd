...
4. **1SE rule for rank selection** (parsimony principle)
5. **1SE rule for lambda selection** (sparsity with stability)
6. **Incremental checkpointing** (saves after each bootstrap iteration)
7. **Resume capability** (continue from last checkpoint if interrupted)
8. **Parallel execution** (multi-core speedup)

**Two-Stage Parameter Selection:**

**Stage 1 - Rank Selection (1SE Rule):**
- Evaluate all ranks at λ=0.0 (no regularization)
- Find minimum mean SSE and its standard error (SE)
- Calculate 1SE threshold: min_SSE + 1×SE
- **Select smallest rank with SSE ≤ threshold** (most parsimonious)
- Rationale: Avoids overfitting by preferring simpler models when performance is comparable

**Stage 2 - Lambda Selection (1SE Rule):**
- At the selected rank, evaluate all lambda values
- Find maximum FMS and its standard error
- Calculate 1SE threshold: max_FMS - 1×SE
- **Select maximum lambda with FMS ≥ threshold** (most sparse)
- Rationale: Maximizes sparsity while maintaining factor stability

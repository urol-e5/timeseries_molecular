...
# Load results from disk if all_rank_results doesn't exist
if 'all_rank_results' not in globals():
    print("Loading results from disk (all_rank_results not in memory)...")
    all_rank_results = []
...
if len(successful) == 0:
    ...
    for r in successful:
        if hasattr(r['model'], 'loss_') and r['model'].loss_ is not None:
            loss = r['model'].loss_
...
## Quick comparison summary in R

```{r view-rank-comparison, eval=FALSE}
# Load and compare metrics across ranks
comparison_file <- file.path(output_dir, "rank_comparison", "rank_comparison_metrics.csv")

if(file.exists(comparison_file)) {
  metrics <- read.csv(comparison_file)
  
  cat("\n=== RANK COMPARISON SUMMARY ===\n")
  print(metrics[, c("Rank", "variance_explained", "relative_error", 
                    "gene_sparsity", "converged")])
  
  cat("\n=== TOP 3 RANKS BY VARIANCE EXPLAINED ===\n")
  top3 <- metrics[order(-metrics$variance_explained), ][1:3, ]
  print(top3[, c("Rank", "variance_explained", "gene_sparsity", "n_iterations")])
  
  cat("\n=== RECOMMENDATION ===\n")
  # Find rank with best balance
  # (high variance explained, high sparsity, converged)
  metrics$score <- metrics$variance_explained * 0.4 +
                   metrics$gene_sparsity * 0.3 +
                   (as.numeric(metrics$converged) * 0.3)
  
  best <- metrics[which.max(metrics$score), ]
  cat("Recommended rank:", best$Rank, "\n")
  cat("  Variance explained:", round(best$variance_explained * 100, 1), "%\n")
  cat("  Gene sparsity:", round(best$gene_sparsity * 100, 1), "%\n")
  cat("  Converged:", best$converged, "\n")
  
} else {
  cat("No rank comparison results found. Run the comparison first.\n")
}
```
...
All results saved to: `../output/13.00-multiomics-barnacle/`

### Rank comparison results:
- `rank_comparison/` - Comparison visualizations and metrics
  - `elbow_plot.png` - **Most important: variance explained vs rank**
  - `rank_comparison_summary.png` - Multi-panel comparison of all metrics
  - `loss_trajectories.png` - Convergence speed by rank
  - `rank_comparison_metrics.csv` - Complete metrics table
- `rank_XX/` - Individual rank results (one directory per rank tested)
  - Each contains: gene_factors.csv, sample_factors.csv, time_factors.csv, etc.

## Next steps for interpretation

1. **Review elbow plot** - Identify where variance explained plateaus
2. **Check recommended rank** - Review automated recommendation output
3. **Examine chosen rank results** - Navigate to `rank_XX/` directory for optimal rank
4. **Biological interpretation** - Analyze top genes, temporal patterns, species effects
5. **GO enrichment** - Perform functional enrichment on component-specific genes
6. **Validation** - Verify patterns match expected biology

## How to use the chosen rank

After identifying the optimal rank from comparison:

1. The complete results are already in `rank_XX/` directory (where XX = chosen rank)
2. Load factor matrices from that directory for detailed analysis
3. Optional: Re-run with single-rank settings (enable `run-barnacle` chunk) for additional visualizations

## Troubleshooting

- **All ranks show low variance explained?** Check data normalization and tensor construction
- **Didn't converge for some ranks?** Increase `n_iter_max` in comparison chunk
- **Unclear which rank to choose?** Focus on elbow in variance explained + biological interpretability

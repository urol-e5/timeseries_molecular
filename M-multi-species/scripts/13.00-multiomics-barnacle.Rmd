...
### 1. Full Dissertation Grid Search (RECOMMENDED - Most Faithful)
**Location:** See sections below starting with "Full Dissertation Grid Search"
- Tests all rank × lambda combinations simultaneously
- Calculates both SSE and FMS for each combination
- Applies two-stage selection: (1) min SSE at λ=0.0 for rank, (2) 1SE rule for lambda
- **Pros:** Complete methodology, captures rank×lambda interactions
- **Cons:** Slower (tests every combination)
- **Use when:** You want the most faithful implementation of Blaskowski (2024)

### 2. Sequential Approach (LEGACY - Faster)
**Location:** Marked as "LEGACY SECTIONS" below
- Tests ranks with λ=0.0, then tests lambdas at optimal rank
- Calculates SSE for rank, FMS for lambda (separately)
- Same selection criteria but no joint optimization
- **Pros:** Faster, simpler, more practical
- **Cons:** May miss rank×lambda interactions
- **Use when:** You need quick results or have limited compute

**Dissertation Reference:** Blaskowski (2024), Section 1.2.3

The dissertation methodology validates rank and lambda selection through:
1. **Stage 1**: Select rank R at minimum cross-validated SSE (λ=0.0 models)
2. **Stage 2**: Select lambda λ at maximum value where FMS ≥ (max_FMS - 1SE)

Both approaches implement this logic, but the grid search does it with full joint testing.

---

### [LEGACY] Identify biological replicate groups

**NOTE: This approach is NO LONGER USED. See "Split-Half Bootstrap Grid Search" below for current recommended approach.**

The leave-one-out cross-validation approach below was replaced based on consultation with the dissertation author (Blaskowski). For limited replicate scenarios, split-half CV with bootstrap resampling provides more stable and interpretable parameter selection.

```{python identify-replicate-groups, eval=TRUE}
# ===========================================
# IDENTIFY BIOLOGICAL REPLICATE GROUPS
# ===========================================
# ⚠️ LEGACY CODE - NOT USED IN CURRENT ANALYSIS
# This code supports the original leave-one-out CV approach.
# Current approach uses split-half bootstrap (see below).
# ===========================================
...
### [CURRENT] Split-Half Bootstrap Grid Search (RECOMMENDED)

**APPROACH:** Following consultation with dissertation author, we implement:
1. **Split-half cross-validation** (instead of leave-one-out)
2. **Bootstrap resampling** (repeated random splits)
3. **Stability assessment** across bootstrap iterations
4. **Incremental checkpointing** (saves after each bootstrap iteration)
5. **Resume capability** (continue from last checkpoint if interrupted)
6. **Two-stage selection**: SSE for rank, FMS for lambda

This approach is recommended when you don't have enough replicates to create completely independent training/test tensors.
...
def split_half_bootstrap_grid_search(
    ...
    print("Criterion: Minimum mean SSE at λ=0.0")
    print("="*80)
    ...
    optimal_rank_sse = lambda_zero_df.loc[optimal_rank_idx, 'mean_sse']
    optimal_rank_se = lambda_zero_df.loc[optimal_rank_idx, 'se_sse']
    
    print(f"\n✓ OPTIMAL RANK: {optimal_rank}")
    print(f"  Mean SSE: {optimal_rank_sse:.2e} ± {optimal_rank_se:.2e}")
    print(f"  (averaged across {n_bootstrap} bootstrap iterations)")
    
    print(f"\nAll λ=0.0 results:")
    display_df = lambda_zero_df[['rank', 'mean_sse', 'se_sse', 'n_converged']].copy()
    display_df = display_df.sort_values('mean_sse')
...
if not os.path.exists(results_file):
    ...
    with open(os.path.join(bootstrap_dir, 'optimal_parameters.json'), 'r') as f:
    ...
    print(f"  Bootstrap iterations: {optimal_params['n_bootstrap_iterations']}")
    ...
    # Mark optimal
    if optimal_rank is not None and optimal_lambda is not None:
        rank_pos = list(plot_df.index).index(optimal_rank)
    ...
    if optimal_rank is not None and optimal_lambda is not None:
        if optimal_lambda in plot_df_fms.columns:
            rank_pos = list(plot_df_fms.index).index(optimal_rank)
    ...
    # ========================================
    # FIGURE 3: Rank Selection (λ=0.0)
    # ========================================
    lambda_zero = results_df[results_df['lambda'] == 0.0].copy()
    lambda_zero = lambda_zero.sort_values('rank')
    
    # Calculate relative SSE (normalized to lowest rank)
    baseline_sse = lambda_zero.iloc[0]['mean_sse']
    lambda_zero['relative_sse'] = lambda_zero['mean_sse'] / baseline_sse
    lambda_zero['relative_se'] = lambda_zero['se_sse'] / baseline_sse
    ...
    # Panel A: Relative SSE (normalized to rank 5)
    ax1 = axes[0]
    ax1.errorbar(lambda_zero['rank'], lambda_zero['relative_sse'],
               yerr=lambda_zero['relative_se'],
               marker='o', markersize=10, linewidth=2.5,
               capsize=5, capthick=2,
               color='darkviolet', ecolor='plum')
    
    if optimal_rank is not None:
        ax1.axvline(x=optimal_rank, color='red', linestyle='--',
    ...
    ax1.set_ylabel(f'Relative SSE (normalized to rank {int(lambda_zero.iloc[0]["rank"])})', fontsize=12)
    ax1.set_title('Stage 1: Rank Selection at λ=0.0\n' +
                f'Based on {optimal_params["n_bootstrap_iterations"]} Bootstrap Iterations',
                fontsize=13, fontweight='bold')
    ax1.legend(fontsize=11, loc='upper right')
    ax1.grid(alpha=0.3)
    ax1.set_ylim([0.75, 1.05])
    
    # Panel B: Coefficient of Variation (shows relative uncertainty is consistent)
    ax2 = axes[1]
    ax2.plot(lambda_zero['rank'], lambda_zero['cv'] * 100,
            marker='s', markersize=8, linewidth=2,
            color='teal', label='CV across bootstrap iterations')
    
    if optimal_rank is not None:
        ax2.axvline(x=optimal_rank, color='red', linestyle='--',
                  linewidth=2, alpha=0.7)
    ...
    ax2.set_title('Relative Bootstrap Variability (CV = σ/μ)',
                fontsize=11, fontweight='bold')
    ax2.legend(fontsize=10, loc='upper right')
    ax2.grid(alpha=0.3)
    ax2.set_ylim([0, max(lambda_zero['cv'] * 100) * 1.2])
    
    plt.tight_layout()
    plt.savefig(os.path.join(fig_dir, 'bootstrap_rank_selection.png'),
                dpi=300, bbox_inches='tight')
    plt.show()
    plt.close()
    ...
    # ========================================
    if optimal_rank is not None:
        optimal_rank_data = results_df[results_df['rank'] == optimal_rank].copy()
        ...
        ax1.set_title(f'SSE vs Lambda at Optimal Rank (R={optimal_rank})',
                     fontsize=13, fontweight='bold')
        ...
        if len(fms_data) > 0:
            ...
            ax2.set_title(f'Stage 2: Lambda Selection at Optimal Rank (R={optimal_rank})\n' +
...
### LEGACY: Run full dissertation grid search (Leave-One-Out CV)

**NOTE:** This is the original leave-one-out approach. The split-half bootstrap approach above is now **RECOMMENDED** based on consultation with the dissertation author.

**WORKFLOW NOTE**: This chunk saves results to files, allowing you to:
1. Run grid search (this chunk) - may take hours
2. Close notebook / R session
3. Come back later
4. Run visualization chunk independently (loads from files)

```{python run-dissertation-grid-search, eval=FALSE}
# ===========================================
# RUN FULL DISSERTATION GRID SEARCH
# ===========================================
# Tests all rank × lambda combinations
# Calculates both SSE and FMS for each
# Applies two-stage selection process
# ===========================================

print("="*80)
print("STARTING FULL DISSERTATION GRID SEARCH")
print("Blaskowski (2024) Section 1.2.3")
print("="*80)
...
# Rank range to test
# Dissertation tested R ∈ [1, 2, 3, ..., 12]
# For computational efficiency, start with coarser grid
GRID_RANK_RANGE = [5, 10, 15, 20, 25, 30, 35]

# Lambda values to test
# Dissertation tested λ ∈ [0.0, 0.05, 0.1, 0.2, 0.4, 0.8, 1.6, 3.2, 6.4, 12.8]
# Start with similar range
GRID_LAMBDA_VALUES = [0.0, 0.01, 0.05, 0.1, 0.5, 1.0, 2.0, 5.0]

# Other parameters
GRID_N_ITER_MAX = 10000
GRID_RANDOM_STATE = 42

# ============================================
# CHOOSE CV APPROACH
# ============================================
# SPECIES-LEVEL CV (RECOMMENDED - Fast, 3 folds)
GRID_CV_GROUPS = species_groups

# Alternatively: COLONY-LEVEL CV (Slower, ~30-40 folds)
# GRID_CV_GROUPS = replicate_groups

print(f"\nGrid search configuration:")
print(f"  Ranks to test ({len(GRID_RANK_RANGE)}): {GRID_RANK_RANGE}")
print(f"  Lambdas to test ({len(GRID_LAMBDA_VALUES)}): {GRID_LAMBDA_VALUES}")
print(f"  Total combinations: {len(GRID_RANK_RANGE)} × {len(GRID_LAMBDA_VALUES)} = {len(GRID_RANK_RANGE) * len(GRID_LAMBDA_VALUES)}")
print(f"  CV folds: {len(GRID_CV_GROUPS)}")
print(f"  Max iterations: {GRID_N_ITER_MAX}")
print(f"  Random state: {GRID_RANDOM_STATE}")
print("="*80)

# Estimate runtime
models_per_combination = len(GRID_CV_GROUPS)
total_models = len(GRID_RANK_RANGE) * len(GRID_LAMBDA_VALUES) * models_per_combination
print(f"\nEstimated workload:")
print(f"  Models to fit: {total_models}")
print(f"  ({len(GRID_RANK_RANGE)} ranks × {len(GRID_LAMBDA_VALUES)} lambdas × {models_per_combination} CV folds)")
print(f"  Estimated time: ~{total_models * 2 / 60:.1f}-{total_models * 5 / 60:.1f} minutes")
print(f"    (assuming 2-5 min per model)")
print("="*80)

# NOTE: Set eval=FALSE above to prevent accidental execution
# When ready to run, set eval=TRUE and execute this chunk
print("\n" + "="*80)
...
optimal_rank_grid, optimal_lambda_grid, grid_results_df = dissertation_grid_search_cv(
    tensor=tensor_3d,
    replicate_groups=GRID_CV_GROUPS,
    rank_range=GRID_RANK_RANGE,
    lambda_values=GRID_LAMBDA_VALUES,
    n_iter_max=GRID_N_ITER_MAX,
    random_state=GRID_RANDOM_STATE
)
...
print("="*80)
print(f"Total time: {elapsed_time / 60:.1f} minutes")
print(f"Time per combination: {elapsed_time / (len(GRID_RANK_RANGE) * len(GRID_LAMBDA_VALUES)):.1f} seconds")
print("="*80)

# Save results
grid_search_dir = os.path.join(output_dir, 'dissertation_grid_search')
os.makedirs(grid_search_dir, exist_ok=True)

# Save full grid results
grid_results_df.to_csv(
    os.path.join(grid_search_dir, 'full_grid_results.csv'),
    index=False
)
print(f"\n✓ Full grid results saved to: full_grid_results.csv")

# Save optimal parameters
import json
with open(os.path.join(grid_search_dir, 'optimal_parameters.json'), 'w') as f:
    json.dump({
        'method': 'dissertation_grid_search',
        'reference': 'Blaskowski 2024, Section 1.2.3',
        'optimal_rank': int(optimal_rank_grid) if optimal_rank_grid is not None else None,
        'optimal_lambda': float(optimal_lambda_grid) if optimal_lambda_grid is not None else None,
        'rank_range_tested': GRID_RANK_RANGE,
        'lambda_values_tested': GRID_LAMBDA_VALUES,
        'cv_method': 'leave_one_group_out',
        'n_cv_folds': len(GRID_CV_GROUPS),
        'cv_groups': list(GRID_CV_GROUPS.keys()),
        'total_combinations': len(GRID_RANK_RANGE) * len(GRID_LAMBDA_VALUES),
...
print(f"\n{'='*80}")
print("Results saved to directory:")
print(f"  {grid_search_dir}")
print("="*80)
```

### Visualize grid search results

**INDEPENDENT CHUNK**: This chunk loads results from saved files and can be run:
- Immediately after grid search completes
- Hours/days later in a fresh R session
- Multiple times to regenerate plots
- Without needing the grid search to be in memory

**Required files** (created by grid search chunk):
- `dissertation_grid_search/full_grid_results.csv`
- `dissertation_grid_search/optimal_parameters.json`

```{python plot-grid-search-results, eval=FALSE}
# ===========================================
# VISUALIZE FULL GRID SEARCH RESULTS
# ===========================================
# This chunk can be run independently after grid search completes
# It loads results from saved files
# ===========================================
...
results_file = os.path.join(grid_search_dir, 'full_grid_results.csv')
params_file = os.path.join(grid_search_dir, 'optimal_parameters.json')

if not os.path.exists(results_file):
    print(f"ERROR: Grid search results file not found: {results_file}")
    print("Please run the grid search chunk first.")
else:
    print(f"Loading results from: {results_file}")
    
    # Load grid search results from file
    grid_results_df = pd.read_csv(results_file)
    print(f"✓ Loaded {len(grid_results_df)} grid search results")
    
    # Load optimal parameters from file
    if os.path.exists(params_file):
        with open(params_file, 'r') as f:
            optimal_params = json.load(f)
        optimal_rank_grid = optimal_params['optimal_rank']
        optimal_lambda_grid = optimal_params['optimal_lambda']
        print(f"✓ Loaded optimal parameters: rank={optimal_rank_grid}, lambda={optimal_lambda_grid}")
    else:
        print("WARNING: Optimal parameters file not found. Plots will not show optimal selections.")
        optimal_rank_grid = None
        optimal_lambda_grid = None
    
    # Clean data for plotting
    plot_df = grid_results_df.copy()
    plot_df = plot_df[~plot_df['mean_cv_sse'].isna()]
    
    print(f"\nPlotting {len(plot_df)} combinations with valid results")
    
    # ============================================
    # Figure 1: SSE Heatmap (Rank × Lambda)
    # ============================================
    fig, ax = plt.subplots(figsize=(12, 8))
    
    # Pivot for heatmap
    sse_pivot = plot_df.pivot(index='rank', columns='lambda', values='mean_cv_sse')
    
    # Plot heatmap
    sns.heatmap(sse_pivot, annot=True, fmt='.2e', cmap='viridis_r',
                cbar_kws={'label': 'Mean CV-SSE'},
                ax=ax)
    
    # Mark optimal rank (from λ=0.0)
    if optimal_rank_grid is not None:
        lambda_zero_idx = list(sse_pivot.columns).index(0.0) if 0.0 in sse_pivot.columns else 0
        rank_idx = list(sse_pivot.index).index(optimal_rank_grid) if optimal_rank_grid in sse_pivot.index else 0
        ax.add_patch(plt.Rectangle((lambda_zero_idx, rank_idx), 1, 1,
                                   fill=False, edgecolor='red', lw=3))
    
    # Mark optimal lambda at optimal rank
    if optimal_rank_grid is not None and optimal_lambda_grid is not None:
        if optimal_lambda_grid in sse_pivot.columns and optimal_rank_grid in sse_pivot.index:
            lambda_idx = list(sse_pivot.columns).index(optimal_lambda_grid)
            rank_idx = list(sse_pivot.index).index(optimal_rank_grid)
            ax.add_patch(plt.Rectangle((lambda_idx, rank_idx), 1, 1,
                                       fill=False, edgecolor='cyan', lw=3))
    
    ax.set_title('Cross-Validated SSE: Rank × Lambda Grid Search\n' + 
                 f'Red box = Optimal rank (R={optimal_rank_grid}, λ=0.0)\n' +
                 f'Cyan box = Final selection (R={optimal_rank_grid}, λ={optimal_lambda_grid})',
                 fontsize=14, fontweight='bold')
    ax.set_xlabel('Lambda (Regularization)', fontsize=12)
    ax.set_ylabel('Rank', fontsize=12)
    
    plt.tight_layout()
    plt.savefig(os.path.join(grid_search_dir, 'grid_search_sse_heatmap.png'),
                dpi=300, bbox_inches='tight')
    plt.show()
    plt.close()
    
    print("✓ SSE heatmap saved")
    
    # ============================================
    # Figure 2: FMS Heatmap (Rank × Lambda)
    # ============================================
    plot_df_fms = plot_df[~plot_df['mean_cv_fms'].isna()].copy()
    
    if len(plot_df_fms) > 0:
        fig, ax = plt.subplots(figsize=(12, 8))
        
        # Pivot for heatmap
        fms_pivot = plot_df_fms.pivot(index='rank', columns='lambda', values='mean_cv_fms')
        
        # Plot heatmap
        sns.heatmap(fms_pivot, annot=True, fmt='.3f', cmap='RdYlGn',
                    vmin=0, vmax=1,
                    cbar_kws={'label': 'Mean CV-FMS'},
                    ax=ax)
        
        # Mark optimal parameters
        if optimal_rank_grid is not None and optimal_lambda_grid is not None:
            if optimal_lambda_grid in fms_pivot.columns and optimal_rank_grid in fms_pivot.index:
                lambda_idx = list(fms_pivot.columns).index(optimal_lambda_grid)
                rank_idx = list(fms_pivot.index).index(optimal_rank_grid)
                ax.add_patch(plt.Rectangle((lambda_idx, rank_idx), 1, 1,
                                           fill=False, edgecolor='blue', lw=3))
        
        ax.set_title('Cross-Validated FMS: Rank × Lambda Grid Search\n' + 
                     f'Blue box = Final selection (R={optimal_rank_grid}, λ={optimal_lambda_grid})',
                     fontsize=14, fontweight='bold')
        ax.set_xlabel('Lambda (Regularization)', fontsize=12)
        ax.set_ylabel('Rank', fontsize=12)
        
        plt.tight_layout()
        plt.savefig(os.path.join(grid_search_dir, 'grid_search_fms_heatmap.png'),
                    dpi=300, bbox_inches='tight')
        plt.show()
        plt.close()
        
        print("✓ FMS heatmap saved")
    else:
        print("⚠ No FMS data available for plotting")
    
    # ============================================
    # Figure 3: Line plots at optimal rank
    # ============================================
    if optimal_rank_grid is not None:
        optimal_rank_data = plot_df[plot_df['rank'] == optimal_rank_grid].copy()
        optimal_rank_data = optimal_rank_data.sort_values('lambda')
        
        fig, axes = plt.subplots(2, 1, figsize=(12, 10))
        
        # Plot 1: SSE vs Lambda
        ax1 = axes[0]
        ax1.errorbar(optimal_rank_data['lambda'], 
                    optimal_rank_data['mean_cv_sse'],
                    yerr=optimal_rank_data['std_cv_sse'],
                    marker='o', markersize=10, linewidth=2.5,
                    capsize=5, capthick=2,
                    color='steelblue', ecolor='lightblue')
        
        ax1.axvline(x=optimal_lambda_grid, color='red', linestyle='--',
                   linewidth=2, alpha=0.7, label=f'Optimal λ = {optimal_lambda_grid}')
        
        ax1.set_xlabel('Lambda (Regularization)', fontsize=12)
        ax1.set_ylabel('Mean CV-SSE', fontsize=12)
        ax1.set_title(f'SSE vs Lambda at Optimal Rank (R={optimal_rank_grid})',
                     fontsize=13, fontweight='bold')
        ax1.legend(fontsize=11)
        ax1.grid(alpha=0.3)
        
        # Plot 2: FMS vs Lambda
        ax2 = axes[1]
        fms_data = optimal_rank_data[~optimal_rank_data['mean_cv_fms'].isna()]
        
        if len(fms_data) > 0:
            ax2.errorbar(fms_data['lambda'],
                        fms_data['mean_cv_fms'],
                        yerr=fms_data['std_cv_fms'],
                        marker='o', markersize=10, linewidth=2.5,
                        capsize=5, capthick=2,
                        color='forestgreen', ecolor='lightgreen')
            
            # Add 1SE threshold line
            max_fms = fms_data['mean_cv_fms'].max()
            max_fms_row = fms_data[fms_data['mean_cv_fms'] == max_fms].iloc[0]
            fms_1se = max_fms - max_fms_row['se_cv_fms']
            
            ax2.axhline(y=fms_1se, color='orange', linestyle='--',
                       linewidth=2, alpha=0.7, label=f'1SE threshold = {fms_1se:.3f}')
            ax2.axvline(x=optimal_lambda_grid, color='red', linestyle='--',
                       linewidth=2, alpha=0.7, label=f'Optimal λ = {optimal_lambda_grid}')
            
            ax2.set_xlabel('Lambda (Regularization)', fontsize=12)
            ax2.set_ylabel('Mean CV-FMS', fontsize=12)
            ax2.set_title(f'FMS vs Lambda at Optimal Rank (R={optimal_rank_grid})\n' + 
                         'Selection: Maximum λ where FMS ≥ (max_FMS - 1SE)',
                         fontsize=13, fontweight='bold')
            ax2.legend(fontsize=11)
            ax2.grid(alpha=0.3)
            ax2.set_ylim(0, 1.05)
        else:
            ax2.text(0.5, 0.5, 'No FMS data available',
                    ha='center', va='center', fontsize=14,
                    transform=ax2.transAxes)
        
        plt.tight_layout()
        plt.savefig(os.path.join(grid_search_dir, 'optimal_rank_lambda_selection.png'),
                    dpi=300, bbox_inches='tight')
        plt.show()
        plt.close()
        
        print("✓ Lambda selection plot saved")
    
    # ============================================
    # Figure 4: Rank selection (λ=0.0 only)
    # ============================================
    lambda_zero_data = plot_df[plot_df['lambda'] == 0.0].copy()
    lambda_zero_data = lambda_zero_data.sort_values('rank')
    
    if len(lambda_zero_data) > 0:
        fig, ax = plt.subplots(figsize=(12, 6))
        
        ax.errorbar(lambda_zero_data['rank'],
                   lambda_zero_data['mean_cv_sse'],
                   yerr=lambda_zero_data['std_cv_sse'],
                   marker='o', markersize=10, linewidth=2.5,
                   capsize=5, capthick=2,
                   color='darkviolet', ecolor='plum')
        
        if optimal_rank_grid is not None:
            ax.axvline(x=optimal_rank_grid, color='red', linestyle='--',
                      linewidth=2, alpha=0.7, label=f'Optimal rank = {optimal_rank_grid}')
        
        ax.set_xlabel('Rank', fontsize=12)
        ax.set_ylabel('Mean CV-SSE (λ=0.0)', fontsize=12)
        ax.set_title('Rank Selection: Minimum CV-SSE at λ=0.0\n' +
                    'Dissertation Stage 1: Select R at minimum error without regularization',
                    fontsize=13, fontweight='bold')
        ax.legend(fontsize=11)
        ax.grid(alpha=0.3)
        
        plt.tight_layout()
        plt.savefig(os.path.join(grid_search_dir, 'rank_selection_lambda_zero.png'),
                    dpi=300, bbox_inches='tight')
        plt.show()
        plt.close()
        
        print("✓ Rank selection plot saved")
...
## LEGACY SECTIONS (Sequential Rank → Lambda)

**NOTE:** The sections below implement a **simplified sequential approach** where rank and lambda are selected separately. This is faster but may miss rank×lambda interactions.

**NEW APPROACH:** The **Full Dissertation Grid Search** (above) implements the complete methodology from Blaskowski (2024) where all rank×lambda combinations are tested simultaneously with both SSE and FMS metrics calculated for each.

**Use these legacy sections** if you want a faster, more practical approach.  
**Use the dissertation grid search** for the most faithful implementation.

---

### Sequential rank selection (Legacy method)

This section is kept for backward compatibility and faster exploratory analysis.

**NOTE:** For the full dissertation grid search, use `dissertation_grid_search_cv()` function above. This legacy function only tests ranks with λ=0.0.

```{python sequential-rank-selection-function, eval=TRUE}
# ===========================================
# SEQUENTIAL RANK SELECTION FUNCTION (LEGACY)
# ===========================================
# Simplified version for backward compatibility
# For full grid search, use dissertation_grid_search_cv() above
# ===========================================
...
def cross_validated_rank_selection(
    ...
    """
    LEGACY: Sequential rank selection via cross-validation.
    
    This is a simplified version that only tests ranks with λ=0.0.
    For the full dissertation grid search, use dissertation_grid_search_cv().
    ...
    print("="*80)
    print("CROSS-VALIDATED RANK SELECTION (LEGACY SEQUENTIAL)")
...
print("Sequential rank selection function loaded (legacy compatibility)")
```

### Run sequential rank selection (Legacy)

```{python run-cv-rank-selection-legacy, eval=FALSE}
# ===========================================
# SEQUENTIAL RANK SELECTION (LEGACY)
# ===========================================
# This is the LEGACY sequential approach
# Use dissertation_grid_search_cv() above for full grid search
# ===========================================

print("="*80)
print("SEQUENTIAL RANK SELECTION (LEGACY METHOD)")
print("For full dissertation grid search, use chunks above")
print("="*80)
            ...
            for i in range(len(fold_decompositions)):
                for j in range(i+1, len(fold_decompositions)):
                    try:
                        ...
                        # NOTE: Sample factors (middle matrix) have different dimensions
                        # across folds, so we compare only gene (0) and time (2) factors
                        
                        # Create tuple of (weights, factors) for each decomposition
                        # Only include gene and time factors (skip sample factors)
                        cp_tensor_i = (
                            fold_decompositions[i].weights,
                            [fold_decompositions[i].factors[0],  # genes
                             fold_decompositions[i].factors[2]]  # time
                        )
                        cp_tensor_j = (
                            fold_decompositions[j].weights,
                            [fold_decompositions[j].factors[0],  # genes
                             fold_decompositions[j].factors[2]]  # time
                        )
                        
                        # Calculate FMS on gene+time factors only
                        fms = factor_match_score(
                            cp_tensor_i, 
                            cp_tensor_j,
                            return_permutation=False
...
### Plot FMS vs SSE for CV results (Sequential Approach - Legacy)

**NOTE:** This section is part of the **legacy sequential approach** where lambda is selected separately after rank selection. See "Full Dissertation Grid Search" above for the complete joint optimization.

After determining the optimal rank (from rank-only CV), this optional diagnostic recreates plots from the dissertation showing the relationship between Factor Match Score (FMS) and reconstruction error (SSE).
    ...
    if len(successful_decompositions) >= 2:
        ...
        for i in range(len(successful_decompositions)):
            for j in range(i+1, len(successful_decompositions)):
                try:
                    # Compare only gene and time factors (skip sample factors due to dimension mismatch)
                    # Create tuple of (weights, factors) format that factor_match_score expects
                    cp_tensor_i = (
                        successful_decompositions[i].weights,
                        [successful_decompositions[i].factors[0],  # genes
                         successful_decompositions[i].factors[2]]  # time

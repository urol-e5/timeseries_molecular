...
# In this dataset: different colonies of the same species at the same timepoint
# ===========================================
...
def identify_replicate_groups_for_cv(sample_labels, species_sample_map, n_timepoints=4):
    ...
    IMPORTANT: For timeseries tensor data (genes × samples × timepoints):
    - Each "sample" dimension = ONE COLONY across all its timepoints
    - Biological replicates = different COLONIES (not different timepoints!)
    - Cross-validation should be leave-one-COLONY-out (not leave-one-timepoint-out)
    
    This function creates groups where each group = ONE COLONY.
    CV will train on all-colonies-except-one, test on the held-out colony.
    
    NOTE: This is NOT the same as biological replicates at the same timepoint.
    In this tensor structure, we're testing if the decomposition can generalize
    to a new colony's timeseries pattern.
    ...
    print(f"  - Each sample = ONE COLONY across {n_timepoints} timepoints")
    print(f"  - CV approach: Leave-one-COLONY-out")
    print(f"\nIMPORTANT CLARIFICATION:")
    print(f"  ✓ Each sample already contains timeseries for one colony")
    print(f"  ✓ We are NOT looking for replicates at same timepoint")
    print(f"  ✓ We are testing if decomposition generalizes to new colonies")
...
CV_RANK_RANGE = [5, 10, 15, 20]  # Start with quick test, expand if needed

            ...
            for i in range(len(fold_decompositions)):
                for j in range(i+1, len(fold_decompositions)):
                    try:
                        ...
                        print(f"      Debug: Comparing fold {i+1} vs fold {j+1}")
                        print(f"        Gene factors: {factors_i[0].shape} vs {factors_j[0].shape}")
                        print(f"        Time factors: {factors_i[1].shape} vs {factors_j[1].shape}")
                        
                        # Calculate FMS on gene+time factors only
                        fms_result = factor_match_score(
                            factors_i, 
                            factors_j,
                            return_permutation=False
                        )
                        
                        print(f"        FMS result type: {type(fms_result)}")
                        print(f"        FMS result value: {fms_result}")
                        
                        # Handle the result
                        if isinstance(fms_result, tuple):
                            fms = fms_result[0]
                        else:
                            fms = fms_result

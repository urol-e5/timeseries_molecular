...
```{python visualize-results, eval=TRUE}
# Create a set of standard visualizations for interpreting Barnacle results
import os
import pandas as pd
import numpy as np
import matplotlib.pyplot as plt
...
# Load data (prefer in-memory variables if present, otherwise read CSVs)
...
# 2) Timepoint loadings (line plot)
...
# 3) Sample-factor heatmap grouped by species
if sample_factor_df is not None and not sample_factor_df.empty:
  try:
    ...
    sns.heatmap(sample_vals_sorted, cmap='vlag', center=0)
    ...
    # 3b) PCA of sample factors (2D scatter)
  ...
  print('No sample_factors available; skipping sample plots')

# 4) Top genes per component (barplots)
...
print('\nVisualization generation complete. Review PNGs in the figures directory.')
```
```

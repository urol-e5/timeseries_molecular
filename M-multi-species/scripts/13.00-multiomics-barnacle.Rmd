...
def calculate_factor_match_score(tensor, decomposition, rank):
    ...
    Higher scores indicate better factor recovery.
    """
    try:
        ...
        try:
            ...
            print(f"  TLViz successfully imported for rank {rank}")
        except ImportError as e:
            print(f"  WARNING: tlviz not available for rank {rank}: {e}")
            print(f"  Attempting to install tensorly-viz...")
            import subprocess
            import sys
            try:
                subprocess.check_call([sys.executable, "-m", "pip", "install", "tensorly-viz", "--upgrade"])
                import tlviz.factor_tools
                print(f"  TLViz successfully installed and imported for rank {rank}")
            except Exception as install_error:
                print(f"  Failed to install/import tlviz: {install_error}")
                return {
                    'fms': np.nan,
                    'permutation': None,
                    'success': False,
                    'error': f"Installation failed: {install_error}",
                    'method': 'failed'
                }
        
        # Create a reference CP tensor from the original data
        # We'll use the decomposition itself as a synthetic "ground truth"
        # This gives us a measure of reconstruction quality
        reference_cp = tl.cp_tensor.CPTensor((decomposition.weights, decomposition.factors))
        
        # Calculate factor match score between decomposition and reference
        # Note: This compares the decomposition's internal consistency
        ...
        error_msg = f"SciPy compatibility issue: {e}"
        if '_lazywhere' in str(e):
            error_msg = "SciPy version incompatibility with tensorly-viz (_lazywhere import error)"
        print(f"  TLViz FMS skipped for rank {rank}: {error_msg}")
        return {
            'fms': np.nan,
            'permutation': None,
            'success': False,
        ...
        print(f"  Error calculating FMS for rank {rank}: {e}")
        return {
            'fms': np.nan,
            'permutation': None,
            'success': False,
...
def calculate_reconstruction_fms(tensor, decomposition, rank):
    """
    Alternative FMS calculation based on reconstruction quality.
    This creates a synthetic ground truth and measures how well 
    the decomposition recovers it.
    """
    try:
...
def evaluate_fms_for_rank(rank, output_dir, tensor):
    ...
    if fms_result1['success']:
        ...
        if 'scipy_incompatibility' in fms_result1.get('method', ''):
            print(f"  TLViz FMS: Skipped (SciPy version incompatibility)")
        else:
            print(f"  TLViz FMS: Failed ({fms_result1.get('error', 'unknown error')})")
...
if tensor_3d is not None:
    ...
    if len(all_fms_results) > 0:
        ...
        if not tlviz_working:
            ...
            print("This is a common issue with newer SciPy versions.")
            print("The Reconstruction FMS provides equivalent functionality and is working correctly.")
        ...
        if len(valid_df) > 0:
            ...
            print(f"\nFMS Interpretation:")
            print(f"• Reconstruction FMS measures tensor reconstruction quality")
            print(f"  (correlation between original and decomposed tensors)")
            print(f"• Reconstruction correlation shows direct fit quality")
            print(f"• Higher scores indicate better factor recovery and model fit")
            print(f"• Reconstruction FMS is the primary metric to use for rank selection")
            if len(tlviz_valid) == 0:
                print(f"• TLViz FMS unavailable due to SciPy version incompatibility")
                print(f"  (This is common and does not affect analysis quality)")
            print(f"• Consider ranks with consistently high Reconstruction FMS")

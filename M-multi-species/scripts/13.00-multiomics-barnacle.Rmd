    ...
    for _, row in cv_rank_results.iterrows():
        ...
        for fold_info in successful_folds[:5]:  # Limit to first 5 folds to save time
            ...
            try:
                ...
                fold_factors.append(decomposition.factors)
                fold_sse.append(sse)
        ...
        if len(fold_factors) >= 2:
            fms_scores = []
            
            for i in range(len(fold_factors)):
                for j in range(i+1, len(fold_factors)):
                    try:
                        # Calculate FMS between factor sets
                        # FMS compares how similar two decompositions are
                        # factor_match_score returns (fms_score, permutation)
                        fms_result = factor_match_score(fold_factors[i], fold_factors[j])
                        
                        # Handle different return types
                        if isinstance(fms_result, tuple):
                            fms = fms_result[0]  # First element is the score
                        else:
                            fms = fms_result  # Direct score
...
for lambda_val in LAMBDA_VALUES:
    ...
    for fold_idx, (group_name, test_indices) in enumerate(USE_CV_GROUPS.items()):
        ...
        try:
            ...
            print(f"    ✗ Error: {e}")
            fold_factors.append(None)
    
    # Calculate pairwise FMS between successful folds
    successful_factors = [f for f in fold_factors if f is not None]
    
    if len(successful_factors) >= 2:
        fms_scores = []
        
        for i in range(len(successful_factors)):
            for j in range(i+1, len(successful_factors)):
                try:
                    # factor_match_score returns (fms_score, permutation)
                    fms_result = factor_match_score(successful_factors[i], successful_factors[j])
        ...
        if len(fms_scores) > 0:
            ...
            lambda_results.append({
                ...
                'se_fms': se_fms,
                'n_comparisons': len(fms_scores),
                'n_successful_folds': len(successful_factors)
            })
            
            print(f"  → Mean FMS: {mean_fms:.4f} ± {std_fms:.4f} (SE = {se_fms:.4f})")
            print(f"     {len(fms_scores)} pairwise comparisons from {len(successful_factors)} folds")
        else:
            print(f"  → No FMS scores calculated")
    else:
        print(f"  → Insufficient successful folds: {len(successful_factors)}")

...
if 'cv_rank_results' not in locals() or cv_rank_results is None:
    ...
    for _, row in cv_rank_results.iterrows():
        ...
        if len(fold_decompositions) >= 2:
            ...
            # Debug: check structure of decomposition
            print(f"    Debug: decomposition type = {type(fold_decompositions[0])}")
            print(f"    Debug: factors type = {type(fold_decompositions[0].factors)}")
            print(f"    Debug: number of factor matrices = {len(fold_decompositions[0].factors)}")
            if hasattr(fold_decompositions[0], 'factors'):
                for idx, factor in enumerate(fold_decompositions[0].factors):
                    print(f"      Factor {idx} shape: {factor.shape if hasattr(factor, 'shape') else 'N/A'}")
            ...
            for i in range(len(fold_decompositions)):
                for j in range(i+1, len(fold_decompositions)):
                    try:
                        ...
                        # Extract gene and time factors (skip sample factors)
                        factors_i = [fold_decompositions[i].factors[0],  # genes
                                    fold_decompositions[i].factors[2]]  # time
                        factors_j = [fold_decompositions[j].factors[0],  # genes
        ...
        for i in range(len(successful_decompositions)):
            for j in range(i+1, len(successful_decompositions)):
                try:
                    ...
                    factors_i = [successful_decompositions[i].factors[0],  # genes
                                successful_decompositions[i].factors[2]]  # time
                    factors_j = [successful_decompositions[j].factors[0],  # genes

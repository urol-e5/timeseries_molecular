...
apul_genes = set(apul_norm['gene_id'])
peve_genes = set(peve_norm['gene_id'])
ptua_genes = set(ptua_norm['gene_id'])
...
apul_common = apul_norm[apul_norm['gene_id'].isin(common_genes_list)].set_index('gene_id').reindex(common_genes_list)
peve_common = peve_norm[peve_norm['gene_id'].isin(common_genes_list)].set_index('gene_id').reindex(common_genes_list)
ptua_common = ptua_norm[ptua_norm['gene_id'].isin(common_genes_list)].set_index('gene_id').reindex(common_genes_list)
...
# Parse sample names to extract sample ID and timepoint information
def parse_sample_names(columns, species_prefix):
    """Parse sample column names to extract sample ID and timepoint"""
    sample_map = {}
    sample_ids = set()
    timepoints = set()
    
    for col in columns:
        # Expected format: ACR.139.TP1, etc.
        parts = col.split('.')
        if len(parts) >= 3 and parts[2].startswith('TP'):
            sample_id = f"{parts[0]}.{parts[1]}"  # e.g., ACR.139
    ...
    return sample_map, sorted(sample_ids), sorted(timepoints)

# Parse sample information for each species
print("Parsing sample information...")

# Use Apul as reference for sample/timepoint structure
sample_map, sample_ids, timepoints = parse_sample_names(apul_common.columns, "ACR")
print(f"Found {len(sample_ids)} unique samples: {sample_ids[:5]}...")
print(f"Found {len(timepoints)} timepoints: {timepoints}")

# Check if all species have the same sample structure
species_data = {
    'apul': apul_common,
    'peve': peve_common, 
    'ptua': ptua_common
...
print(f"\nSample structure consistency check:")
for species, data in species_data.items():
    ...
    for col in data.columns:
        ...
        if len(parts) >= 3 and parts[2].startswith('TP'):
            ...
            species_samples.append(sample_id)
    unique_samples = set(species_samples)
    print(f"{species}: {len(unique_samples)} unique samples")
```

## Create 4D tensor (genes × samples × timepoints × species)

```{python create-4d-tensor, eval=TRUE}
# Create a 4D tensor: genes × samples × timepoints × species
n_genes = len(common_genes_list)
n_samples = len(sample_ids)
n_timepoints = len(timepoints)
n_species = 3

print(f"Creating 4D tensor with shape: ({n_genes}, {n_samples}, {n_timepoints}, {n_species})")

# Initialize tensor
tensor_4d = np.zeros((n_genes, n_samples, n_timepoints, n_species))

# Fill tensor for each species
species_list = ['apul', 'peve', 'ptua']
for species_idx, species in enumerate(species_list):
    data = species_data[species]
    print(f"Processing {species} (species index {species_idx})...")
    
    for sample_idx, sample_id in enumerate(sample_ids):
        for time_idx, timepoint in enumerate(timepoints):
            # Construct column name
            if species == 'apul':
                col_name = f"ACR.{sample_id.split('.')[1]}.TP{timepoint}"
            elif species == 'peve':
                col_name = f"POC.{sample_id.split('.')[1]}.TP{timepoint}"
            elif species == 'ptua':
                col_name = f"POC.{sample_id.split('.')[1]}.TP{timepoint}"
            
            # Find matching column (allowing for slight naming differences)
            matching_cols = [col for col in data.columns if f".{sample_id.split('.')[1]}." in col and f"TP{timepoint}" in col]
            
            if matching_cols:
                col_name = matching_cols[0]
                tensor_4d[:, sample_idx, time_idx, species_idx] = data[col_name].values
            else:
                print(f"Warning: No data found for {species} sample {sample_id} timepoint {timepoint}")
                tensor_4d[:, sample_idx, time_idx, species_idx] = np.nan

# Check for missing data
n_missing = np.sum(np.isnan(tensor_4d))
print(f"\nTensor created with {n_missing} missing values")
print(f"Tensor shape: {tensor_4d.shape}")
print(f"Non-zero values: {np.sum(tensor_4d != 0)}")
print(f"Sparsity: {np.sum(tensor_4d == 0) / tensor_4d.size:.2%}")
```
...
tensor_filled = np.nan_to_num(tensor_4d, nan=0.0)
...
lambdas = [0.1, 0.0, 0.1, 0.1]  # Sparsity penalties for [genes, samples, timepoints, species]
...
model = SparseCP(
    ...
    n_initializations=5,  # Multiple random starts for robustness
    random_state=42
)
...
for i, factor in enumerate(decomposition.factors):
    factor_names = ['Genes', 'Samples', 'Timepoints', 'Species']
...
sample_factors = decomposition.factors[1]  # samples × components  
time_factors = decomposition.factors[2]  # timepoints × components
species_factors = decomposition.factors[3]  # species × components
...
time_df = pd.DataFrame(
    time_factors,
    index=[f'TP{tp}' for tp in timepoints],
    columns=[f'Component_{i+1}' for i in range(rank)]
)
print(time_df)

print("\n=== SAMPLE LOADINGS (first 10 samples) ===")
sample_df = pd.DataFrame(
    sample_factors[:10],  # Show first 10 samples
    index=sample_ids[:10],
    columns=[f'Component_{i+1}' for i in range(rank)]
)
print(sample_df)
...
# Species factors  
species_df.to_csv(os.path.join(factors_dir, "species_factors.csv"))
...
metadata = {
    ...
    'final_loss': float(model.loss_[-1]),
    'n_iterations': len(model.loss_),
    'species_order': species_list,
    'timepoint_order': timepoints,
    'sample_order': sample_ids[:10],  # First 10 for brevity
...
print("- sample_factors.csv: Sample loadings for each component") 
print("- time_factors.csv: Timepoint loadings for each component")
print("- species_factors.csv: Species loadings for each component")
...
cat("   - Created 4D tensor: genes × samples × timepoints × species\n")

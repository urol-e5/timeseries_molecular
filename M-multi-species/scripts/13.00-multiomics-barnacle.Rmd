...
rank = 20  # Number of components to extract
lambdas = [0.1, 0.0, 0.1]  # Sparsity penalties for [genes, samples, timepoints]
n_iter_max = 1000  # Maximum number of iterations

# Create model with all available parameters
# (Parameters marked with "# DEFAULT" show the default value from barnacle)
model = SparseCP(
    rank=rank,                          # REQUIRED: Number of components
    lambdas=lambdas,                    # REQUIRED: L1 sparsity coefficients per mode
    nonneg_modes=[0],                   # DEFAULT: None - List of modes forced to be non-negative
    norm_constraint=True,               # DEFAULT: True - Constrain L2 norm of non-sparse factors
    init='random',                      # DEFAULT: 'random' - Initialization method ('random' or CPTensor)
    tol=0.1,                          # DEFAULT: 1e-6 - Convergence tolerance
    n_iter_max=n_iter_max,                   # DEFAULT: 1000 - Maximum iterations (using 20000 here)
    random_state=40,                    # DEFAULT: None - Random seed for reproducibility
    n_initializations=1                 # DEFAULT: 1 - Number of random initializations
)
...
try:
    ...
    if hasattr(model, 'loss_') and model.loss_ is not None:
        ...
        print(f"\nMax iterations set: {n_iter_max}")
        print(f"Iterations completed: {len(loss_history)}")
        
        # Determine convergence status
        if len(loss_history) < n_iter_max:
            print(f"Convergence: Algorithm converged early (met tolerance criterion of {model.tol})")
        else:
            print(f"Convergence: Algorithm reached maximum iterations without converging")
        
        print(f"Initial loss: {loss_history[0]:.6f}")
        print(f"Final loss: {loss_history[-1]:.6f}")
        print(f"Loss reduction: {loss_history[0] - loss_history[-1]:.6f} ({(1 - loss_history[-1]/loss_history[0])*100:.2f}% decrease)")
    else:
...
if decomposition_success:
    ...
    if loss_history is not None:
        loss_df = pd.DataFrame({
            'Iteration': list(range(1, len(loss_history) + 1)),
            'Loss': loss_history
        })
        loss_df.to_csv(os.path.join(factors_dir, "loss_history.csv"), index=False)
        print("- loss_history.csv: Loss values per iteration")
...
```{python viz-loss-convergence, eval=TRUE}
# Loss convergence plot
loss_path = os.path.join(factors_dir, 'loss_history.csv')
if os.path.exists(loss_path):
    try:
        ...
        ax = plt.gca()
        ax.plot(loss_df['Iteration'], loss_df['Loss'], linewidth=2, color='steelblue')
        ax.set_xlabel('Iteration')
        ax.set_ylabel('Loss')
        ax.set_title('Convergence: Loss per Iteration')
        ...
        plt.tight_layout()
        out = os.path.join(fig_dir, 'loss_convergence.png')
        ...
        pct_reduction = (loss_reduction / initial_loss) * 100
        
        print(f'\nConvergence Statistics:')
        print(f'  Iterations: {n_iters}')
        print(f'  Initial loss: {initial_loss:.6f}')
        print(f'  Final loss: {final_loss:.6f}')
        print(f'  Loss reduction: {loss_reduction:.6f} ({pct_reduction:.2f}%)')
        ...
        print('Loss convergence plot failed:', e)
else:
    print('No loss_history.csv found; skipping convergence plot')
```

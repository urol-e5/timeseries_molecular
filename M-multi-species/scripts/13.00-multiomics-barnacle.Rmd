...
def identify_replicate_groups_for_cv(sample_labels, species_sample_map, n_timepoints=4):
    ...
    For timeseries data with structure (genes × samples × timepoints):
    - Each "sample" represents a colony across all timepoints
    - Biological replicates = different colonies of same species
    - Each replicate group = set of colonies from one species
    ...
    replicate_groups : dict
        Keys: species name
        Values: list of sample indices for that species
    """
    ...
    print("="*60)
    print("BIOLOGICAL REPLICATE GROUPS FOR CROSS-VALIDATION")
    print("="*60)
    print(f"Total samples: {len(sample_labels)}")
    print(f"Timepoints per sample: {n_timepoints}")
    print(f"\nReplicate groups (by species):")
    
    for species, indices in sorted(groups.items()):
        print(f"  {species}: {len(indices)} biological replicates")
        # Show first few sample IDs
        sample_ids = [species_sample_map[i]['sample_id'] for i in indices[:3]]
        print(f"    Examples: {sample_ids}...")
    
    print(f"\nTotal replicate groups: {len(groups)}")
    print(f"Suitable for {len(groups)}-fold leave-one-species-out cross-validation")
    print("="*60)
...
CV_RANK_RANGE = [5, 10, 15, 20, 25, 30, 35, 40]  # Adjust as needed
...
if optimal_rank_cv is not None:
    ...
    with open(os.path.join(cv_results_dir, 'optimal_rank_cv.json'), 'w') as f:
        json.dump({
            ...
            'cv_method': 'leave_one_species_out',
            'n_cv_folds': len(replicate_groups),

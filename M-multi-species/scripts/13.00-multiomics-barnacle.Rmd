...
def split_half_bootstrap_grid_search(
    ...
    1. For each bootstrap iteration:
       ...
       - Fit models at all rank × lambda combinations on training set
    ...
    for boot_idx in range(start_iter, n_bootstrap):
        ...
        for rank_idx, R in enumerate(rank_range):
            for lambda_idx, lambda_val in enumerate(lambda_values):
                combo_num = rank_idx * len(lambda_values) + lambda_idx + 1
                total_combos = len(rank_range) * len(lambda_values)
                
                print(f"\n  [{combo_num}/{total_combos}] Rank={R}, Lambda={lambda_val}")
                ...
                try:
                    ...
                    # Compare gene and time factors (sample factors have different dimensions)
                    try:
                        cp_train = (
                    ...
                    print(f"    ✓ SSE={sse_test:.2e}, FMS={fms:.4f}")
...
**Workflow:**
1. Run this chunk (may take several hours depending on n_bootstrap)
...
# Rank range to test
BOOTSTRAP_RANK_RANGE = [5, 10, 15, 20, 25, 30, 35]
...
# Run split-half bootstrap grid search with incremental saving
...
if not os.path.exists(results_file):
    ...
    ax.errorbar(lambda_zero['rank'], lambda_zero['mean_sse'],
    ...
    if optimal_rank is not None:
        ax.axvline(x=optimal_rank, color='red', linestyle='--',
                  linewidth=2, alpha=0.7,
                  label=f'Optimal Rank = {optimal_rank}')
    
    ax.set_xlabel('Rank', fontsize=12)
    ax.set_ylabel('Mean SSE (± SE)', fontsize=12)
    ax.set_title('Stage 1: Rank Selection at λ=0.0\n' +

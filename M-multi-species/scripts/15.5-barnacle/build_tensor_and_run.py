...
def main() -> None:
    ...
    # Use the merged CSV file instead of separate species files
    merged_file = os.path.join(input_dir, 'vst_counts_matrix.csv')
    if not os.path.exists(merged_file):
        raise FileNotFoundError(f"Missing merged input file: {merged_file}")

    # Read the merged CSV file (without setting index yet)
    merged_df = pd.read_csv(merged_file)
    if 'group_id' not in merged_df.columns:
        raise ValueError(f"Expected 'group_id' column in {merged_file}")

    # Split the data by species based on sample prefixes
    species_to_df = {}
    species_prefixes = {
        'apul': ['ACR'],  # ACR prefix for apul
        'peve': ['POR'],  # POR prefix for peve
        'ptua': ['POC']   # POC prefix for ptua
    }

    for species, prefixes in species_prefixes.items():
        species_cols = [col for col in merged_df.columns if any(col.startswith(prefix) for prefix in prefixes)]
        if species_cols:
            species_to_df[species] = merged_df[['group_id'] + species_cols].copy()
            species_to_df[species] = species_to_df[species].set_index('group_id')

    # Check that we found data for all expected species
    missing_species = set(species_prefixes.keys()) - set(species_to_df.keys())
    if missing_species:
        found_prefixes = set()
        for col in merged_df.columns:
            for prefixes in species_prefixes.values():
                if any(col.startswith(prefix) for prefix in prefixes):
                    found_prefixes.update(prefixes)
        raise ValueError(f"Missing data for species: {missing_species}. Found prefixes: {found_prefixes}")
    ...
    save_outputs(output_dir, tensor, sample_labels, species_sample_map, genes, model, decomposition, args.rank)

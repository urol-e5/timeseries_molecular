...
lipid_quant_long<-lipid_quant_long%>%
  ...
  mutate(protein.ug=if_else(sample=="L26", 340.2, 
                         if_else(sample=="L39", 139.2, 
                                 if_else(sample=="L67", 7.3, 
                                         if_else(sample=="L80", 35.0, 
                                                 if_else(sample=="L84", 185.9, NA))))))
```
...
saveRDS(data, file = "M-multi-species/data/lipidomics/lipids.rds")
```

Output data count matrices. 
```{r}
head(data)

output<-data%>%
  mutate(sample_id=paste0(colony, "-", timepoint))%>%
  select(!c(sample, colony, timepoint, species, type))%>%
  pivot_wider(names_from=sample_id, values_from=nmol.g_plasma.ug_protein)%>%
  as.data.frame()

rownames(output)<-output$lipid
...
write.csv(output, file="M-multi-species/output/lipid_count_matrix.csv")
```

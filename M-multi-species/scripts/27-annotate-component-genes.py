#!/usr/bin/env python3
"""
Annotate component gene CSV files with ortholog group annotations.
...
def annotate_component_file(component_file_path, annotation_df, output_dir):
    """
    Annotate a single component CSV file with ortholog annotations.
    
    Args:
        component_file_path: Path to the component CSV file
    ...
    component_df_renamed = component_df.rename(columns={og_column: 'group_id'})
    
    # Merge with annotations
    annotated_df = component_df_renamed.merge(
        annotation_df, 
        on='group_id', 
        how='left'
    )
    ...
    input_filename = os.path.basename(component_file_path)
...
def main():
    ...
    output_dir = component_dir  # Save annotated files in the same directory
    summary_output = component_dir / 'annotation_summary.md'
    ...
    # Find all CSV files in the component directory (excluding annotation files)
    csv_files = [f for f in component_dir.glob('*.csv') 
    ...
    for component_file in sorted(csv_files):
        try:
            print(f"Processing {component_file.name}...")
            output_path, summary = annotate_component_file(component_file, annotation_df, output_dir)
    ...
    with open(summary_output, 'w') as f:
        f.write("# Component Gene List Annotation Summary\n\n")

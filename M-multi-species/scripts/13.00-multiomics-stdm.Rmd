...
This notebook will use Steven's [`workflow-stdm`](https://github.com/sr320/workflow-stdm) (GitHub repo) tensor decomposition project.
...
# CONDA
conda_env_name <- c("/home/sam/programs/mambaforge/envs/sr320_stdm_py312_env")
conda_path <- c("/opt/anaconda/anaconda3/bin/conda")
```

## Load [`workflow-stdm`](https://github.com/sr320/workflow-stdm) conda environment

If this is successful, the first line of output should show that the Python being used is the one in your [`workflow-stdm`](https://github.com/sr320/workflow-stdm) conda environment path.

E.g.

`python:         /home/sam/programs/mambaforge/envs/sr320_stdm_py312_env/bin/python`
...
```{python analyze-rank-comparison, eval=TRUE}
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
...
## Run tensor decomposition with optimal rank

Based on the rank comparison analysis, let's run the final decomposition with the suggested rank:

```{python run-optimal-decomposition, eval=TRUE}
import subprocess
import os
import json
...
# Find the main results directory and load rank selection summary
...
if os.path.exists(summary_file):
    ...
    suggested_rank = rank_summary['suggested_rank']
    
    print(f"Using suggested rank: {suggested_rank}")
    print(f"Based on: {'Elbow point analysis' if rank_summary['elbow_rank'] else 'Best reconstruction error'}")
    
    # Use the transformed data file
    input_file = os.path.join(output_dir, "vst_counts_matrix_long_format.csv")
    
    # Create output directory for final analysis within main results
    final_output_dir = os.path.join(main_results_dir, f"final_analysis_rank_{suggested_rank}")
    os.makedirs(final_output_dir, exist_ok=True)
    
    print(f"Running final stdm decomposition with rank {suggested_rank}...")
    print(f"Output directory: {final_output_dir}")
    
    # Run stdm with optimal rank
    subprocess.run([
        "stdm", "run",
        "--input", input_file,
        "--output", final_output_dir,
        "--rank", str(suggested_rank),
        "--method", "parafac",
        "--sparsity-threshold", "0.01",
        "--no-log-transform",
        "--standardize",
    ...
    print(f"Final decomposition completed with rank {suggested_rank}")
    
else:
    print("Rank selection summary not found. Using default rank 8 for demonstration.")
    suggested_rank = 8
...
# Find the main results directory and final analysis directory
if 'main_results_dir' in globals():
    main_results_dir = globals()['main_results_dir']
else:
    # Fallback to find the most recent main results directory
...
# Find the final analysis directory within main results
final_analysis_dirs = glob.glob(os.path.join(main_results_dir, "final_analysis_rank_*"))
if final_analysis_dirs:
    final_analysis_dir = max(final_analysis_dirs)  # Get most recent
    print(f"Analyzing optimal results from: {final_analysis_dir}")
    
    # Find the most recent results directory within final analysis
    result_dirs = glob.glob(os.path.join(final_analysis_dir, "20*"))
    if result_dirs:
        ...
        # Store latest_dir for subsequent analyses
        globals()['optimal_results_dir'] = latest_dir
        
    else:
        print("No timestamped results directories found in final analysis!")
        
else:
    print("No final analysis directories found!")
    # Fallback to original results in main directory
    result_dirs = glob.glob(os.path.join(main_results_dir, "20*"))
    if result_dirs:
        globals()['optimal_results_dir'] = max(result_dirs)
        print(f"Using fallback results from: {globals()['optimal_results_dir']}")
    else:
...
```{python generate-all-rank-visualizations, eval=TRUE}
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
import seaborn as sns
...
```{python visualize-optimal-results, eval=TRUE}
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
import seaborn as sns
...
# Use the optimal results directory (set in previous chunk)
if 'optimal_results_dir' in globals() and optimal_results_dir:
    latest_dir = optimal_results_dir
    print(f"Using optimal analysis data from: {latest_dir}")
else:
    print("Error: optimal_results_dir not found. Please run the 'Analyze optimal decomposition results' chunk first.")
    latest_dir = None
...
if latest_dir:
    ...
    # Save the plot
    plot_file = os.path.join(latest_dir, "optimal_tensor_decomposition_visualization.png")
...
```{python plot-component-strengths, eval=TRUE}
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
import glob
...
# Use the optimal results directory (set in previous chunk)
# Only use final_analysis_rank_* directories - no fallbacks
final_analysis_dir = None
latest_dir = None
...
# Only proceed if we find final_analysis_rank_* directories
if main_results_dir:
    final_analysis_dirs = glob.glob(os.path.join(main_results_dir, "final_analysis_rank_*"))
    if final_analysis_dirs:
        final_analysis_dir = max(final_analysis_dirs)
        # Look for timestamped results within the final analysis directory
        result_dirs = glob.glob(os.path.join(final_analysis_dir, "20*"))
        if result_dirs:
            latest_dir = max(result_dirs)
            print(f"Using final analysis data from: {latest_dir}")
        else:
            print("No timestamped results found in final_analysis_rank_* directory!")
    else:
        print("No final_analysis_rank_* directories found!")
else:
...
if latest_dir:
    ...
    # Save individual plot in final analysis directory
    if final_analysis_dir:
        plot_file = os.path.join(final_analysis_dir, "plot_01_component_strengths.png")
        plt.savefig(plot_file, dpi=300, bbox_inches='tight')
...
```{python plot-temporal-heatmap, eval=TRUE}
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
import seaborn as sns
...
# Use the optimal results directory and final analysis directory
if 'optimal_results_dir' in globals():
    latest_dir = optimal_results_dir
    if 'main_results_dir' in globals():
        main_results_dir = globals()['main_results_dir']
        final_analysis_dirs = glob.glob(os.path.join(main_results_dir, "final_analysis_rank_*"))
        final_analysis_dir = max(final_analysis_dirs) if final_analysis_dirs else latest_dir
    else:
        final_analysis_dir = latest_dir
else:
    # Fallback logic (same as plot 1)
    main_result_dirs = glob.glob(os.path.join(output_dir, "20*"))
    if main_result_dirs:
        main_results_dir = max(main_result_dirs)
        final_analysis_dirs = glob.glob(os.path.join(main_results_dir, "final_analysis_rank_*"))
        if final_analysis_dirs:
            final_analysis_dir = max(final_analysis_dirs)
            result_dirs = glob.glob(os.path.join(final_analysis_dir, "20*"))
            latest_dir = max(result_dirs) if result_dirs else None
        else:
            latest_dir = None
            final_analysis_dir = None
    else:
...
if latest_dir:
    ...
    sorted_components = np.argsort(component_strengths)[::-1]
    top_5_components = sorted_components[:5]
    ...
    sns.heatmap(time_data.T, 
                ...
                linewidths=0.5)
    plt.title('Top 5 Components: Temporal Patterns')
    plt.xlabel('Timepoint')
    plt.ylabel('Component')
    
    # Save individual plot in final analysis directory
    if final_analysis_dir:
        plot_file = os.path.join(final_analysis_dir, "plot_02_temporal_heatmap.png")
        plt.savefig(plot_file, dpi=300, bbox_inches='tight')
...
```{python plot-species-heatmap, eval=TRUE}
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
import seaborn as sns
...
if latest_dir:
    ...
    sns.heatmap(species_data.T,
                ...
                linewidths=0.5)
    plt.title('Top 5 Components: Species Patterns')
    plt.xlabel('Individual')
    plt.ylabel('Component')
    plt.xticks(rotation=90)
    ...
    # Save individual plot in final analysis directory
    if final_analysis_dir:
        plot_file = os.path.join(final_analysis_dir, "plot_03_species_heatmap.png")
        plt.savefig(plot_file, dpi=300, bbox_inches='tight')
...
```{python plot-temporal-trajectories, eval=TRUE}
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
import glob
...
# Use the optimal results directory and final analysis directory (same logic as other chunks)
if 'optimal_results_dir' in globals():
    latest_dir = optimal_results_dir
    if 'main_results_dir' in globals():
        main_results_dir = globals()['main_results_dir']
...
if latest_dir:
    ...
    plt.xlabel('Timepoint')
    plt.ylabel('Factor Value')
    plt.title('Temporal Trajectories (Top 5 Components)')
    plt.xticks(range(1, len(timepoint_names) + 1), timepoint_names)
    plt.legend()
    plt.grid(True, alpha=0.3)
    ...
    # Save individual plot in final analysis directory
    if final_analysis_dir:
        plot_file = os.path.join(final_analysis_dir, "plot_04_temporal_trajectories.png")
        plt.savefig(plot_file, dpi=300, bbox_inches='tight')
...
```{python plot-species-clustering, eval=TRUE}
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
from sklearn.cluster import KMeans
...
if latest_dir:
    ...
    # Calculate component strengths and get top 10 for clustering
    component_strengths = []
    for i in range(gene_factors.shape[1]):
        gene_norm = np.linalg.norm(gene_factors[:, i])
        species_norm = np.linalg.norm(species_factors[:, i])
    ...
    # Use top 10 components for clustering
    species_subset = species_factors[:, sorted_components[:10]]
    
    # Create species type labels
    species_types = [name.split('-')[0] for name in species_names]
    unique_types = list(set(species_types))
    ...
    # Save individual plot in final analysis directory
    if final_analysis_dir:
        plot_file = os.path.join(final_analysis_dir, "plot_05_species_clustering.png")
        plt.savefig(plot_file, dpi=300, bbox_inches='tight')
...
```{python plot-gene-distribution, eval=TRUE}
import numpy as np
import matplotlib.pyplot as plt
import glob
import os
...
if latest_dir:
    ...
    # Save individual plot in final analysis directory
    if final_analysis_dir:
        plot_file = os.path.join(final_analysis_dir, "plot_06_gene_distribution.png")
        plt.savefig(plot_file, dpi=300, bbox_inches='tight')
...
```{python plot-component-correlations, eval=TRUE}
import numpy as np
import matplotlib.pyplot as plt
import seaborn as sns
import glob
...
if latest_dir:
    ...
    # Save individual plot in final analysis directory
    if final_analysis_dir:
        plot_file = os.path.join(final_analysis_dir, "plot_07_component_correlations.png")
        plt.savefig(plot_file, dpi=300, bbox_inches='tight')
...
```{python plot-species-comparison, eval=TRUE}
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
import glob
...
if latest_dir:
    ...
    plt.xlabel('Component')
    plt.ylabel('Mean Factor Value')
    plt.title('Species Type Comparison (Top 5 Components)')
    plt.xticks(x_pos + width, [f'Comp {i+1}' for i in top_5_components])
    plt.legend()
    plt.grid(True, alpha=0.3)
    
    # Save individual plot
    plot_file = os.path.join(latest_dir, "plot_08_species_comparison.png")
    plt.savefig(plot_file, dpi=300, bbox_inches='tight')
...
```{python plot-temporal-variance, eval=TRUE}
import numpy as np
import matplotlib.pyplot as plt
import glob
import os
...
if latest_dir:
    ...
    # Save individual plot
    plot_file = os.path.join(latest_dir, "plot_09_temporal_variance.png")
    plt.savefig(plot_file, dpi=300, bbox_inches='tight')
...
```{python plot-species-variance, eval=TRUE}
import numpy as np
import matplotlib.pyplot as plt
import glob
import os
...
if latest_dir:
    ...
    # Save individual plot
    plot_file = os.path.join(latest_dir, "plot_10_species_variance.png")
    plt.savefig(plot_file, dpi=300, bbox_inches='tight')
...
```{python plot-top-genes, eval=TRUE}
import numpy as np
import matplotlib.pyplot as plt
import seaborn as sns
import glob
...
if latest_dir:
    ...
    # Get top genes for each of the top 5 components
    n_top_genes = 10
    top_gene_indices = []
    
    for comp_idx in top_5_components:
        top_genes = np.argsort(np.abs(gene_factors[:, comp_idx]))[-n_top_genes:]
        top_gene_indices.extend(top_genes)
    
    top_gene_indices = list(set(top_gene_indices))  # Remove duplicates
    top_gene_data = gene_factors[top_gene_indices, :][:, top_5_components]
    ...
    sns.heatmap(top_gene_data, 
                ...
                xticklabels=[f'Comp {i+1}' for i in top_5_components],
                cbar_kws={'label': 'Gene factor loading'})
    plt.title(f'Top {len(top_gene_indices)} Genes × Top 5 Components')
    plt.xlabel('Component')
    plt.ylabel('Gene')
    
    # Save individual plot
    plot_file = os.path.join(latest_dir, "plot_11_top_genes.png")
    plt.savefig(plot_file, dpi=300, bbox_inches='tight')
...
```{python plot-reconstruction-quality, eval=TRUE}
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
import glob
...
if latest_dir:
    ...
    # Save individual plot
    plot_file = os.path.join(latest_dir, "plot_12_reconstruction_quality.png")
    plt.savefig(plot_file, dpi=300, bbox_inches='tight')
...
if latest_dir:
    ...
    # Save the interpretation plot in the appropriate directory
    if final_analysis_dir:
        interp_plot_file = os.path.join(final_analysis_dir, "species_clustering_interpretation.png")
    else:

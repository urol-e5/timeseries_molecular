...
if 'optimal_results_dir' in globals():
    ...
    # Fallback to find the most recent results
    result_dirs = glob.glob(os.path.join(output_dir, "20*"))
    if result_dirs:
        latest_dir = max(result_dirs)
    else:
        print("No results directories found!")
...
if 'optimal_results_dir' in globals():
    latest_dir = optimal_results_dir
    # Also get the final analysis directory for saving individual plots
    if 'main_results_dir' in globals():
        main_results_dir = globals()['main_results_dir']
        final_analysis_dirs = glob.glob(os.path.join(main_results_dir, "final_analysis_rank_*"))
        if final_analysis_dirs:
...
if latest_dir:
    ...
    plt.ylabel('Component Strength')
    plt.title('Component Strengths (Gene × Species × Time norms)')
    plt.xticks(range(1, len(component_strengths) + 1, 5))
    plt.grid(True, alpha=0.3)
...
if result_dirs:
    ...
    plt.ylabel('Temporal Variance')
    plt.title('Components by Temporal Variability')
    plt.xticks(range(1, len(time_variances) + 1, 5))
    plt.grid(True, alpha=0.3)
...
if result_dirs:
    ...
    plt.ylabel('Species Variance')
    plt.title('Components by Species Variability')
    plt.xticks(range(1, len(species_variances) + 1, 5))
    plt.grid(True, alpha=0.3)

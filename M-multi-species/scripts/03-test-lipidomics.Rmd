...
plot1<-lipid_quant_long %>%
  ...
  labs(x = "Sample", y = "Total Lipid (nmol per mg tissue)", title = "Total lipid content") +
  theme_classic() +
...
POC has more PUFA content than others. POR noteably has fewer of 20:4, which is arachidonic acid. This is a common coral FFA. 
...
POC has lower saturated FA compostion than the other species. 
...
ACR cluster closely with POR and POC farther away.  
...
class1<-class_quant_long %>%
  ...
  facet_wrap(~ class)+
  geom_bar(stat="identity")+
  labs(x = "Sample", y = "Concentration (nmol per mg tissue)", title = "Lipid class concentration") +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), 
...
```
POC has more TG and POR has fewer TG by composition. There are also fewer PC in POC and more PE in POR. 

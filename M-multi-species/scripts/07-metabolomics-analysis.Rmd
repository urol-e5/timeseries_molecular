...
output<-data%>%
  ...
  select(!c(sample, colony, timepoint, species, site))%>%
  pivot_wider(names_from=sample_id, values_from=rel.quant.ug)%>%
  as.data.frame()

head(output)

rownames(output)<-output$compound
...
write.csv(output, file="M-multi-species/output/metabolomics_count_matrix.csv")
```

...
phys<-phys%>%
  select(sample_id, Host_AFDW.mg.cm2, Sym_AFDW.mg.cm2, Am, AQY, Rd, Ik, Ic, calc.umol.cm2.hr, cells.mgAFDW, prot_mg.mgafdw, cre.umol.mgafdw, Ratio_AFDW.mg.cm2, Total_Chl, Total_Chl_cell)%>%
...
for (col in module_levels) {
  df <- kME_tbl %>%
    ...
    select(group_id, module_label, module_color, kME_in_own)

...
save(net, module_colors, module_labels, MEs, expr_mat, file = "M-multi-species/output/18-ortholog-wgcna/wgcna_ortho.rda")
```
...
save(mod_trait_pval,mod_trait_cor, mod_trait_tbl, trait_mat_plus, file = "M-multi-species/output/18-ortholog-wgcna/wgcna_mod_trait.rda")
```
...
```{r}
#keep only row names starting with ACR 
expr_mat <- expr_mat[grepl("^ACR", rownames(expr_mat)), ]
```
...
#keep only row names starting with ACR 
trait_mat <- trait_mat[grepl("^ACR", rownames(trait_mat)), ]
```
...
#keep only row names starting with ACR 
phys <- phys[grepl("^ACR", phys$sample_id), ]

#remove species columns in trait matrix 
trait_mat <- trait_mat[, -c(7,8,9)]
```
...
# Example: save gene lists per module color
write_csv(gene_module_df, "M-multi-species/output/18-ortholog-wgcna/acropora_wgcna/gene_module_assignments.csv")
...
# Output locations
out_base <- "M-multi-species/output/18-ortholog-wgcna/acropora_wgcna"
out_dir  <- file.path(out_base, "module_gene_lists")
dir.create(out_dir, showWarnings = FALSE, recursive = TRUE)
...
```{r}
#keep only row names starting with POC 
expr_mat <- expr_mat[grepl("^POC", rownames(expr_mat)), ]
```
...
#keep only row names starting with POC 
trait_mat <- trait_mat[grepl("^POC", rownames(trait_mat)), ]
```
...
#keep only row names starting with POC 
phys <- phys[grepl("^POC", phys$sample_id), ]

#remove species columns in trait matrix 
trait_mat <- trait_mat[, -c(7,8,9)]
```

Filter to remove outlier in calcification data. 
```{r}
phys$calc.umol.cm2.hr[phys$calc.umol.cm2.hr > 1.5] <- NA
```
...
# Example: save gene lists per module color
write_csv(gene_module_df, "M-multi-species/output/18-ortholog-wgcna/pocillopora_wgcna/gene_module_assignments.csv")
...
# Output locations
out_base <- "M-multi-species/output/18-ortholog-wgcna/pocillopora_wgcna"
out_dir  <- file.path(out_base, "module_gene_lists")
dir.create(out_dir, showWarnings = FALSE, recursive = TRUE)
...
```{r}
#keep only row names starting with POR 
expr_mat <- expr_mat[grepl("^POR", rownames(expr_mat)), ]
```
...
#keep only row names starting with POR 
trait_mat <- trait_mat[grepl("^POR", rownames(trait_mat)), ]
```
...
#keep only row names starting with POR 
phys <- phys[grepl("^POR", phys$sample_id), ]
...
# Example: save gene lists per module color
write_csv(gene_module_df, "M-multi-species/output/18-ortholog-wgcna/porites_wgcna/gene_module_assignments.csv")
...
# Output locations
out_base <- "M-multi-species/output/18-ortholog-wgcna/porites_wgcna"
out_dir  <- file.path(out_base, "module_gene_lists")
dir.create(out_dir, showWarnings = FALSE, recursive = TRUE)
...
# View list of orthologs in each species that correlate with calcification 

## All species combined WGCNA 

Modules 1, 3, 8, 11, 7, 5, 2

Extract list of orthologs in these modules 
```{r}
all_orthos<-read_csv("M-multi-species/output/18-ortholog-wgcna/wgcna_ortholog_module_assignments.csv")%>%filter(wgcna_module %in% c(1, 3, 8, 11, 7, 5, 2))%>%pull(group_id)
```

## Acropora

Modules 6, 17, 20, 5, 10, 1, 18, 14, 2, 16

Extract list of orthologs in these modules 
```{r}
acr_orthos<-read_csv("M-multi-species/output/18-ortholog-wgcna/acropora_wgcna/wgcna_ortholog_module_assignments.csv")%>%filter(wgcna_module %in% c(6, 17, 20, 5, 10, 1, 18, 14, 2, 16))%>%pull(group_id)
```
...
Extract list of orthologs in these modules 
```{r}
poc_orthos<-read_csv("M-multi-species/output/18-ortholog-wgcna/pocillopora_wgcna/wgcna_ortholog_module_assignments.csv")%>%filter(wgcna_module %in% c(24))%>%pull(group_id)
```

## Porites 

Modules 1, 21, 23, 7, 2, 17, 6, 11, 22

Extract list of orthologs in these modules 
```{r}
por_orthos<-read_csv("M-multi-species/output/18-ortholog-wgcna/porites_wgcna/wgcna_ortholog_module_assignments.csv")%>%filter(wgcna_module %in% c(1, 21, 23, 7, 2, 17, 6, 11, 22))%>%pull(group_id)
```

## Display unique and shared orthologs 

Generate an upset plot showing shared orthologs for modules correlating with calcification 
...
# Plot
pdf("M-multi-species/output/18-ortholog-wgcna/species_orthologs_calcification_comparison_upset.pdf", width = 8, height = 6)
upset(upset_data,
      #order.by = "freq",
      sets = c("All", "ACR", "POC", "POR"),
...
Output list of shared orthologs in all species from individual wgcnas

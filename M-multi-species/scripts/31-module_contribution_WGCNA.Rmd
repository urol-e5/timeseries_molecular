...
dir.create("M-multi-species/output/31-module_contribution_WGCNA/MM", recursive = TRUE, showWarnings = FALSE)

pdf(file = "M-multi-species/output/31-module_contribution_WGCNA/calcification_hubGenes_expression.pdf", width = 9, height = 8)
...
lapply(modNames, function(mod) {
  ...
  # Export
  write.csv(df, file = sprintf("M-multi-species/output/31-module_contribution_WGCNA/MM/moduleMembership_%s.csv", mod), row.names = FALSE)
  write.csv(top_kme, file = sprintf("M-multi-species/output/31-module_contribution_WGCNA/MM/moduleMembership_%s_topTenPct.csv", mod), row.names = FALSE)
})
```
...
# Determine which modules have significant GS–MM relationships
...
pdf(file = "M-multi-species/output/31-module_contribution_WGCNA/calcificationSig_MM.pdf", width = 9, height = 8)

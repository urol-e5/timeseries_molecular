...
# Rscript path/to/10-format-miRNA-counts.R path/to/raw/counts.txt path/to/metadata.csv path/to/output/file.txt
#
...
# Rename columns by matching to metadata (AzentaSampleName → ColonyID_Timepoint)
#  Create mapping

name_map <- metadata_df %>%
  select(AzentaSampleName, ColonyID, Timepoint) %>%
  mutate(new_name = paste0(AzentaSampleName, "_", ColonyID, "_", Timepoint)) %>%

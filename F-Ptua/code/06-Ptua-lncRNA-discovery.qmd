...
DATA_DIR <- "../data/06-Ptua-lncRNA-discovery"
OUTPUT_DIR <- "../output/06-Ptua-lncRNA-discovery"
THREADS <- "46"
  
FASTQ_SOURCE <- "https://gannet.fish.washington.edu/gitrepos/urol-e5/timeseries_molecular/F-Ptua/output/01.00-F-Ptua-RNAseq-trimming-fastp-FastQC-MultiQC/"
FASTQ_SUFFIX <- "fq.gz"
GENOME_SOURCE <- "https://owl.fish.washington.edu/halfshell/genomic-databank/Pocillopora_meandrina_HIv1.assembly.fasta"


GTF_SOURCE <- "https://github.com/urol-e5/timeseries_molecular/raw/d5f546705e3df40558eeaa5c18b122c79d2f4453/F-Ptua/data/Pocillopora_meandrina_HIv1.genes-validated.gtf"
GFF_SOURCE <- "https://raw.githubusercontent.com/urol-e5/timeseries_molecular/d5f546705e3df40558eeaa5c18b122c79d2f4453/F-Ptua/data/Pocillopora_meandrina_HIv1.genes-validated.gff3"
...
FEATURECOUNT_DIR <- "/srlab/programs/subread-2.0.5-Linux-x86_64/bin/"
...
Sys.setenv(
  ...
  FEATURECOUNT_DIR = FEATURECOUNT_DIR
)
```
...
```{r, engine='bash'}
# eval "$(/opt/anaconda/anaconda3/bin/conda shell.bash hook)" <-needed for raven
...
"${FEATURECOUNTS_DIR}featureCounts" \
  ...
  -o "${OUTPUT_DIR}/lncRNA_counts.txt" \
  -t lncRNA \
  -g gene_id \
  -p \
"${OUTPUT_DIR}"/*sorted.bam
...
```{bash}
head "${OUTPUT_DIR}/lncRNA_counts.txt"
```
...
awk -v FS='\t' -v OFS='\t' '
  ...
  NR > 1 { print }
' "${OUTPUT_DIR}/lncRNA_counts.txt" > "${OUTPUT_DIR}/lncRNA_counts.clean.txt"
```

```{bash}
head "${OUTPUT_DIR}/lncRNA_counts.clean.txt"
```
...
```{bash}
python 06.1-filter_count_matrix.py "${OUTPUT_DIR}/lncRNA_counts.clean.txt" "${OUTPUT_DIR}/lncRNA_counts.clean.filtered.txt"
```



```{bash}
head "${OUTPUT_DIR}/lncRNA_counts.clean.filtered.txt"
```


```{bash}
wc -l "${OUTPUT_DIR}/lncRNA_counts.clean.filtered.txt"
```



## Go back and filter GTF based on this

```{bash}
# Create a temporary file with the matching criteria from lncRNA_counts.clean.filtered.txt
# Extract columns 2, 3, 4 (Chr, Start, End) and create a lookup pattern
awk 'NR>1 {print $2"\t"$3"\t"$4}' "${OUTPUT_DIR}/lncRNA_counts.clean.filtered.txt" > "${OUTPUT_DIR}/temp_lookup.txt"

# Filter the GTF file to keep only rows where columns 1, 4, 5 (Chr, Start, End) match the lookup
awk 'BEGIN {
    # Read the lookup file into an associative array
    while ((getline line < "'${OUTPUT_DIR}'/temp_lookup.txt") > 0) {
        lookup[line] = 1
    }
    close("'${OUTPUT_DIR}'/temp_lookup.txt")
}
{
    # Check if columns 1, 4, 5 from GTF (Chr, Start, End) match any entry in lookup
    key = $1"\t"$4"\t"$5
    if (key in lookup) {
        print $0
    }
}' "${OUTPUT_DIR}/lncRNA.gtf" > "${OUTPUT_DIR}/lncRNA.filtered.gtf"

# Clean up temporary file
rm "${OUTPUT_DIR}/temp_lookup.txt"

# Check the results
echo "Original GTF lines:"
wc -l "${OUTPUT_DIR}/lncRNA.gtf"
echo "Filtered GTF lines:"
wc -l "${OUTPUT_DIR}/lncRNA.filtered.gtf"
```

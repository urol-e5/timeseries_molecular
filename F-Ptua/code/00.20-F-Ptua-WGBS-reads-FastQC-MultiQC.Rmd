...
## Compare expected sample files to downloaded files
...
# 1. Make a list of expected files (R1 and R2 for each sample)
expected_files=$(echo "$sample_list" | awk '{print $1"_R1_001.fastq.gz\n"$1"_R2_001.fastq.gz"}' | sort)

# Safely list downloaded files (filenames only, sorted)
downloaded_files=$(find "${raw_reads_dir}" -maxdepth 1 -type f -name "*.fastq.gz" -printf "%f\n" | sort)

# 3. Find missing files
echo ""
echo "${line}"
echo "Checking for missing files:"
comm -23 <(echo "$expected_files") <(echo "$downloaded_files")
echo "${line}"
echo ""
```
